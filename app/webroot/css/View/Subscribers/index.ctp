...
<div class="page-content">
	<div class="page-header">
		...
		<div class="col-lg-2 paddingleftrightzero">
			<?php echo $this->Html->link('Invoices',array('controller'=>'subscriber_invoices','action'=>'index'),array('class'=>'btn btn-sm btn-success pull-right  btn-inverse addinvoice'));?>
		</div>
	</div>
	<!-- /.page-header -->
	<div class="row">
			...
			<div class="table-responsive table-responsivenoscroll tablemobile_overflowhidd">
				...
				<div class="row margin-twenty-zero filterdivmob">
					...
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 nopadding col-lg-2 col-md-4 col-sm-4">                    	                    	                    	
                    	<?php echo $this->Form->input('plan',array('class'=>'form-control selectpicker','data-placeholder'=>'Plans','options'=>array(''=>'Plan Type',$plans)));?>                    	
                     </div>
					...
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 nopadding col-lg-2 col-md-4 col-sm-4">
						<?php echo $this->Form->input('status',array('class'=>'form-control selectpicker','data-placeholder'=>'Status','options'=>array(''=>'Status','Active'=>'Active','Cancelled'=>'Cancelled','Expired'=>'Expired','Pending'=>'Pending','Suspended'=>'Suspended')));?>
					</div>
					
					
                     
					<div class="form-group filed-left margin-bottom-zero clearlefttrespon ">
						<?php echo $this->Js->submit('Filter',array('url'=>array('action'=>'index'),'class'=>'btn btn-sm btn-primary filter-btn','type'=>'submit','update'=>'#content'));?>
					</div>
					<div class="form-group filed-left margin-bottom-zero">
						<?php echo $this->Js->link('Reset',array('action'=>'index'),array('class'=>'btn btn-sm btn-primary filter-btn','update'=>'#content','title'=>'Reset filtered result'));?>
					</div>
				...
				<table id="sample-table-1" class="table table-striped table-bordered table-hover editable-table nodeletspaceup">
					<thead>
						<tr>
							<th><?php echo $this->Paginator->sort('fullname','Subscriber Name',array('update'=>'#content')); ?></th>
							<th class="hidden-480 hidden-column">Subscriber email</th>
							<th class="hidden-480 hidden-column"><?php echo $this->Paginator->sort('SbsSubscriberOrganizationDetail.organization_name','Company Name',array('update'=>'#content')); ?></th>
							<th class="hidden-480"><?php echo $this->Paginator->sort('cpn_subscription_plan_id','Plan Type',array('update'=>'#content')); ?></th>
							<th class="hidden-480"><?php echo $this->Paginator->sort('status','Status',array('update'=>'#content')); ?></th>
							<th>Action</th>
						</tr>
					</thead>
									...
									<?php
										if($permission['_delete'] == 1) {
											if($subscriber['SbsSubscriber']['status'] == 'Cancelled' || $subscriber['SbsSubscriber']['status'] == 'Suspended') {
												echo $this->Form->postLink('<i class="icon-stop bigger-120"></i>',array('controller'=>'users','action'=>'activateSubscriber',$subscriber['SbsSubscriber']['id'],$plan,$company,$subscriberName,$status,$page),array('class'=>'btn btn-xs btn-grey view on-load','title'=>'Activate','escape'=>FALSE,'confirm'=>'Are you sure want to activate subscriber ?'));
											}
											if($subscriber['SbsSubscriber']['status'] == 'Active') {
												echo $this->Form->postLink('<i class="icon-play bigger-120"></i>',array('controller'=>'users','action'=>'cancelSubscriber',$subscriber['SbsSubscriber']['id'],$plan,$company,$subscriberName,$status,$page),array('class'=>'btn btn-xs btn-grey view on-load','title'=>'Deactivate','escape'=>FALSE,'confirm'=>'Are you sure want to deactivate subscriber ?'));
											}
										} 
									?>
								</div>
