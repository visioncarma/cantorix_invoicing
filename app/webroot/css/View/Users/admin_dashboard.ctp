...
<div class="page-content dashboarddd">
			<!--div class="page-header">
				<h2 class="lighter"><?php echo __('Welcome to'); ?></h2>

				<h1 class="lighter defaultcolor h1default"><?php echo __('CantoriX Admin Dashboard'); ?></h2>
			</div-->
			...
			<?php $dynamicClass = 1;?>
					<?php foreach($allPlan as $planId => $planName):?>
						<?php if($dynamicClass == "1"){
							$classDiv1 = "col-lg-4 col-xs-12 col-md-6 col-sm-6 nopaddingleft margintop15";
						}elseif($dynamicClass == "2"){
							$classDiv1 = "col-lg-4 col-xs-12 col-md-6 col-sm-6 margintop15";
						}elseif($dynamicClass == "3"){
							$classDiv1 = "col-lg-4 col-xs-12 col-md-6 col-sm-6 nopaddingright margintop15";
						}?>
					...
					<?php $dynamicClass++;?>
				<?php endforeach;?>
					<div class="col-lg-4  col-xs-12 col-md-6 col-sm-6 margintop15 nopaddingleft">
						<div class="dashboard-box ">
							<div class="padding10 width100 left">
					...
					<div class="col-lg-4 col-xs-12 col-md-6 col-sm-6 margintop15">
						<div class="dashboard-box ">
							<div class="padding10 width100 left">
					...
					<div class="col-lg-4  col-xs-12 col-md-6 col-sm-6 margintop15 nopaddingright">
						<div class="dashboard-box ">
								...
								<table class="table table-striped table-bordered table-hover rt rt1" >
									...
									<tbody>
										<tr >
											...
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
												<td class = "<?php echo $class?>"><?php echo $getSixMonthData[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['Previous Month Subscribers']?></td>
											<?php }?>
										...
										<tr >
											...
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
												<td class = "<?php echo $class?>"><?php echo $getSixMonthData[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['New Subscribers']?></td>
											<?php }?>
										...
										<tr >
											...
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
												<td class = "<?php echo $class?>"><?php if($getRevenueDetails[date("Y-M",strtotime( date('Y-m-01' )." -$i months"))]['CancelledInvoiceCount']){echo $getRevenueDetails[date("Y-M",strtotime( date('Y-m-01' )." -$i months"))]['CancelledInvoiceCount'];} else{ echo "0";}?></td>
											<?php }?>
										</tr>
										<tr >
											...
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
													<?php $total = $getSixMonthData[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['Previous Month Subscribers'] + $getSixMonthData[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['New Subscribers'] - $getRevenueDetails[date("Y-M",strtotime( date('Y-m-01' )." -$i months"))]['CancelledInvoiceCount'];?>
												<td class = "bold <?php echo $class?>"><?php if($total>=0){echo $total;} else{ echo "0";}?></td>
											<?php }?>
										</tr>
									</tbody>
								</table>
										...
										<?php foreach($getSixMonthSubscriberPerSubscription as $planType => $valueArray):?>
											...
											<tr >
												...
												<?php foreach($valueArray as $key=>$value): ?>
													...
													<td class = "<?php echo $classBold;?>"> <?php echo $value;?></td>
													<?php $row++;?>
												<?php endforeach;?>
						...
						<div class="col-xs-12">
							<div class="table-responsive scrollautoo">
								<table class="rt1 table table-striped table-bordered table-hover rt rt1" >
									...
									<tbody>
										<tr >
											<td> <?php echo __('Paid Subscriptions'); ?></td>
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
										...
										<tr >
											...
											<?php for($i=0;$i<=5;$i++){ ?>
												<?php if($i%2>0){$class = "";}?>
												<td class = "bold <?php echo $class?>"><?php if($getRevenueDetails[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['Total']){echo $this->Number->currency($getRevenueDetails[date("Y-M", strtotime("-$i months"))]['Total'],$adminSetting['CpnSetting']['currency_code']);}else{ echo $this->Number->currency("0",$adminSetting['CpnSetting']['currency_code']);}?></td>
											<?php }?>
										</tr>
									</tbody>
								</table>
