	...
	<div class="page-content">
		<div class="page-header">
			...
			<div class="col-lg-2 paddingleftrightzero">
				<?php echo $this->Html->link('<i class="icon-plus"></i>Add New Role',array('controller'=>'usergroups','action'=>'addUsergroup'),array('class'=>'btn btn-sm btn-success pull-right addbutton','escape'=>FALSE));?>
			</div>
			<?php endif;?>
		</div>
		<!-- /.page-header -->
								...
								<td>
									<div class="btn-group hidden-xs hidden-sm visible-md visible-lg">
										<?php echo $this->Form->postLink('<button class="btn btn-xs btn-success view on-load" title="View"><i class="icon-zoom-in bigger-120"></i></button>',
												array('action' => 'viewUsergroup', $usergroup['Aro']['id']), array('escape'=>FALSE)); ?>
										<?php if($permission['_update'] == '1'):?>
											<?php echo $this->Form->postLink('<button class="btn btn-xs btn-info edit on-load" title="Edit"><i class="icon-edit bigger-120"></i></button>',
												array('action' => 'editUsergroup', $usergroup['Aro']['id']), array('escape'=>FALSE)); ?>
										<?php endif;?>
										...
										if($permission['_delete'] == 1) {
											if($subscriber['SbsSubscriber']['status'] == 'Cancelled' || $subscriber['SbsSubscriber']['status'] == 'Suspended') {
												echo $this->Form->postLink('<i class="icon-stop bigger-120"></i>',array('controller'=>'users','action'=>'activateSubscriber',$subscriber['SbsSubscriber']['id'],$plan,$company,$subscriberName,$page),array('class'=>'btn btn-xs btn-grey view on-load','title'=>'Activate','escape'=>FALSE,'confirm'=>'Are you sure want to activate subscriber ?'));
											}
											if($subscriber['SbsSubscriber']['status'] == 'Active') {
												echo $this->Form->postLink('<i class="icon-play bigger-120"></i>',array('controller'=>'users','action'=>'cancelSubscriber',$subscriber['SbsSubscriber']['id'],$plan,$company,$subscriberName,$page),array('class'=>'btn btn-xs btn-grey view on-load','title'=>'Deactivate','escape'=>FALSE,'confirm'=>'Are you sure want to deactivate subscriber ?'));
											}
										} 
									?>
												</li>
