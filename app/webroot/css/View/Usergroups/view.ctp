				...
				<table id="sample-table-1" class="table table-striped table-bordered table-hover editable-table  table_fixed_new rol_users">
					<thead>
						<tr>
							...
							<th >Role</th>
							<th >Active</th>
							<?php if($permissions['_update'] == 1 || $permissions['_delete'] == 1):?><th>Action</th><?php endif;?>
						</tr>
					</thead>
					...
					<tbody>
						...
						<tr id="tr-<?php echo $user['User']['id'];?>">
							<td class="hidden-480">
								...
								<?php echo $this->Form->input($user['User']['id'].'.username',array('placeholder'=>'User Name','value'=>$user['User']['username'],'disabled'=>'disabled'));?>
							...
							<td class="hidden-480" >
								...
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<?php echo $this->Form->input($user['User']['id'].'.email',array('placeholder'=>'Email','value'=>$user['User']['email'],'disabled'=>'disabled'));?>
							...
							<td>
								...
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<span class="on-edit inlineeditselectpick usergroupselect">	
							...
							<td  ><span class="on-load"><?php if($user['User']['active'] == 'Y') {echo 'Yes';} else { echo 'No'; }?></span>
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<span class="on-edit inlineeditselectpick userselect">
							...
							<div class="btn-group">
								...
								<?php endif;?>
								<?php if($permissions['_update'] == 1 && $user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
									<a class="btn btn-xs btn-info edit edit-row on-load" title="Edit">
										<i class="icon-edit bigger-120"></i>
									</a>
								<?php endif;?>
								
								<?php if($permissions['_delete'] == 1 && $user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
