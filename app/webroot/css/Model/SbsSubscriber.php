 ...
 */
	...
	public function getSubscriberBySubscriptionPlanForLastSixMonth(){
		for($i=0;$i<=5;$i++){
			...
			$j = $i+1;
			$finalArray[date("Y-M",strtotime( date( 'Y-m-01' )." -$i months"))]['Previous Month Subscribers'] = $this->find('count',array('conditions'=>array('SbsSubscriber.subscribed_date like'=>date("Y-m",strtotime( date( 'Y-m-01' )." -$j months")).'%')));
