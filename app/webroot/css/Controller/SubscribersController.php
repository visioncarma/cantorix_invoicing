 ...
 */
	class SubscribersController extends AppController {
		...
		public function index($plan=0,$company=0,$subscriberName = 0,$status=0,$page = NULL) {
			...
			$this->Paginator->settings = array('conditions'=>$conditions,'limit'=>$limit,'order'=>$order,
				'fields'=>array('SbsSubscriber.id','SbsSubscriber.fullname','SbsSubscriber.sbs_subscriber_organization_detail_id','SbsSubscriber.cpn_subscription_plan_id',
					'SbsSubscriber.status','SbsSubscriberOrganizationDetail.organization_name'));
		...
		public function changeSubscription() {
			...
			$this->set(compact('plan','customersCount','usersCount','standardPlan','invoices'));
		}
		...
		public function cancelSubscription() {
			...
			$id = $this->Session->read('Auth.User.SbsSubscriber.id');
			if($id) {
				$success = $this->requestAction(array('controller' => 'users','action' => 'suspendSubscriberPaypalRecurringProfile',$id,'Cancel'));
			}
		...
		$bill_start_day = date('d');//
		
		if($initial_amount < $amount_threshold) {
			//$initial_amount   = $initial_amount + $bill_amount;
			$initial_amount		= 0.00;
			$prorata_amount		= 0.00;
			$init_service_tax	= 0.00;
			//$billdateAr   = date("Y-m", strtotime("+2 month"));
			$billdateAr   = date("Y-m", strtotime("+1 month"));
			$bd 		  = explode('-', $billdateAr); 
			$year  = $bd[0];
			$month = $bd[1];
			...
			$billdate   = date("Y-m-d", strtotime("+0 day"));//
			$billBegins = gmdate("$billdate\TH:i:s\Z");
			$profilestartdate = $billBegins;
			$splitRow = 1;			
		} else {
	   	    ...
	   	    if(!empty($check_password)){
	   	    	  if($this->data['User']['new_password'] == $this->data['User']['confirm_password']){
	   	        			 ...
	   	        			 if($this->User->save($this->request->data)){
	   	                          ...
	   	                          $this->Session->write('Auth.User',$userDetails['User']);
	   	                          $this->Session->setFlash(__('<div class="alert alert-block alert-success">Password has been updated!</div>'));
		...
		public function passowrd_check(){
			
			//$password = $this->params->query['current_password'];
			//Configure::write('debug',2);
		...
		public function updateOrganisationDetails() {
			...
			$quote 		= $this->SlsQuotation->find('first',array('fields'=>array('id')));
			$invoice 	= $this->AcrClientInvoice->find('first',array('fields'=>array('id')));
			$inventory	= $this->InvInventory->find('first',array('fields'=>array('id')));
			$expense	= $this->AcpExpense->find('first',array('fields'=>'id'));
			...
			if(!empty($this->data)) {
				...
				if(!empty($this->data['OrganisationProfile']['cpn_currency_id'])) {
					...
					$this->SbsSubscriberSetting->save($curencySetting);
				}
