 ...
 * **/
	public function index() {
		...
		$adminUsergroup = $userGroup->find('first',array('conditions'=>array('Aro.parent_id' => '2','Aro.sbs_subscriber_id'=>$subscriberID)));
		$this->Paginator->settings = array('conditions'=>array('Aro.foreign_key IS NULL','Aro.parent_id'=>2,'Aro.sbs_subscriber_id'=>$subscriberID),'limit'=>$limit,'fields'=>array('id','alias'));
 ...
 */ 	
	public function view($id = null,$subscriberID = NULL,$user_id = null,$delete = NULL) {
		...
		if($subscriberID == $subsID) {
			...
			$this->set(compact('menuActive','settingsActive','menus','title_for_layout','permission','groupName','users','permissions','user_groups','id','subscriberID','adminUser'));
 ...
 * **/	
	public function checkRole($parent_id=NULL) {
 ...
 * **/
 	public function editUsergroup($id = null) {
		...
		$this->request->onlyAllow('post', 'editUsergroup');
		$menus = $this->User->getAllMenus('Super Admin');
		$permission = $this->User->getPermissions($id);
		$Aro = $this->Acl->Aro;
		$Aro->recursive = -1;
 ...
 * **/
	public 	function manageUsergroup()	{
		...
		$this->Paginator->settings = array('conditions'=>array('Aro.foreign_key IS NULL','Aro.parent_id'=>1),'limit'=>$limit,'fields'=>array('id','alias'));
		$usergroups = $this->Paginator->paginate($userGroup);
		foreach($usergroups as $index => $value) {
			$userIDSS = NULL;
			$usergroups[$index] = $value;
 ...
 */
	public function delete($id = null) {
		...
		$Aro->id = $id;
		if (!$Aro->exists()) {
			throw new NotFoundException(__('<div class="alert alert-danger">Invalid Role</div>'));
		}
		...
		if ($Aro->delete()) {
			$this->Session->setFlash(__('<div class="alert alert-block alert-success">Users and user role has been deleted!</div>'));
		} else {
 ...
 */ 	
	public function viewUsergroup($id = null,$page = NULL) {
		...
		$adminUserGroup = $Aro->find('first',array('conditions'=>array('Aro.alias'=>'Admin','Aro.parent_id'=>1),'fields'=>array('Aro.id')));
