...
class AppController extends Controller {
	...
	public $components = array('RequestHandler','Acl','Session','Email','Cookie',
		'Auth' => array(
        'loginAction' => array(
            'controller' => 'users',
            'action' => 'login'
			...
			foreach ($tax_groups_details as $t1 => $t2) {
				...
				foreach($tax_mappings as $m1 => $m2){
					...
					foreach($taxname as $name1 => $name2){
						$final_taxes2[$t2.'-'.$t1][$m2['SbsSubscriberTaxGroupMapping']['id']][$m2['SbsSubscriberTaxGroupMapping']['sbs_subscriber_tax_id']] = $name2['SbsSubscriberTax']['name'].'-'.$name2['SbsSubscriberTax']['percent'].'%';
						$final_taxes[$name2['SbsSubscriberTax']['id']]= $name2['SbsSubscriberTax']['name'].'-'.$name2['SbsSubscriberTax']['percent'].'%';
						$final_taxes[$t2.'-'.$t1][$m2['SbsSubscriberTaxGroupMapping']['sbs_subscriber_tax_group_id']][$m2['SbsSubscriberTaxGroupMapping']['sbs_subscriber_tax_id']] = $name2['SbsSubscriberTax']['name'].'-'.$name2['SbsSubscriberTax']['percent'].'%';
					}
				}
			}
		}else{
			$final_taxes = $this->SbsSubscriberTax->find('list',array('conditions'=>array('SbsSubscriberTax.sbs_subscriber_id'=>$this->subscriber),'order' => 'SbsSubscriberTax.name ASC'));
		}
		ksort($final_taxes);
		$var = 0;		
 ...
 */
	...
	public function generateInvoiceNumber($subscriberId = null) {
		...
		$invoiceFormat 		 = $settings['SbsSubscriberSetting']['invoice_number_format'];
