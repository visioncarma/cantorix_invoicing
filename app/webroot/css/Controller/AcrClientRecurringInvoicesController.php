 ...
 */
	public function index($filterAction = 0, $filterValue = 0, $filterValue1 = 0, $filterValue2 = 0, $isRecurring = 0, $status = 0, $fromDate = 0, $toDate = 0, $page = 1) {			
		...
		$this->set(compact('dateFormat','filterAction','filterValue','filterValue1','filterValue2','isRecurring','status','fromDate','toDate'));
	}	
 ...
 */
	public function view($id = null,$filterAction = 0, $filterValue = 0, $filterValue1 = 0, $filterValue2 = 0,$isRecurring,$status = 0,$fromDate = 0,$toDate = 0,$page) {
		...
		$this->set(compact('acrClientRecurringInvoice','invoiceDetail','invoiceData','subscriberOrganisationDetail'));
	}
	...
	public function add() {
		...
		if ($this->request->is('post')) {
			if(!empty($this->data['AcrClientInvoice']['inventory'])){
				...
				$invoiceDetail['invoiced_date'] 		= $this->data['AcrClientInvoice']['start_date'];
				...
				if($saveInvoice){
					...
					$saveRecuringDetail['invoice_start_date']		=	date('Y-m-d',strtotime($this->data['AcrClientInvoice']['start_date']));
					$saveRecuringDetail['invoice_end_date']			=	date('Y-m-d',strtotime($this->data['AcrClientInvoice']['end_date']));
		...
		$getCustomFields = $this->AcrInvoiceCustomField->getFieldList($this->subscriber);
		$this->set(compact('invoiceNumber','customer', 'inventoryList','currencyList','defaultCurrency','paymentTerm','taxList','defaultCurrencyCode','getCustomFields','dateFormat'));
	}
 ...
 */
	...
	public function edit($id = null,$recurringId = null,$filterAction = 0, $filterValue = 0, $filterValue1 = 0, $filterValue2 = 0,$isRecurring,$status = 0,$fromDate = 0,$toDate = 0,$page) {
						...
						if($inventoryVal){
							...
							if($invDetailId){
								...
								if($updateInventoryStock){
									...
									$recurringInvoiceData['invoice_start_date'] = date('Y-m-d',strtotime($this->data['AcrClientInvoice']['start_date']));
									$recurringInvoiceData['invoice_end_date'] 	= date('Y-m-d',strtotime($this->data['AcrClientInvoice']['end_date']));
		...
		$taxList = $this->taxTree();
		$getCustomFields = $this->AcrInvoiceCustomField->getFieldList($this->subscriber);
		$getCustomFieldsVal = $this->AcrInvoiceCustomValue->getFieldValueByInvoiceId($acrClientRecurringInvoice['AcrClientInvoice']['id']); 
