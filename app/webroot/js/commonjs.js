...
$(document).ready(function(){
	...
	$("#organizationForm").validate({
		...
		onkeyup: false,
		rules: {
			'data[organizationForm][organization_name]': {
				required: true,				
		...
		messages: {
			'data[organizationForm][organization_name]': {
				...
				checkCompanyAvailability:"Company Name Already Exist"
			},
			'data[organizationForm][email_address]': {
	...
	$.validator.addMethod("checkCompanyAvailability",function(value,element){
	 var x= $.ajax({
	    url: "/"+rootDirectory+"/users/isCompanyExist",
	    type: 'GET',
	    async: false,
	    data: "company_name=" + value + "&checking=true",
	 }).responseText;	 	
	 if(x=="true") return false;
	 else return true;
	},"Sorry, this companyis not available");
