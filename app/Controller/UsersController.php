...
class UsersController extends AppController {
	...
	function freeCheckout () {
		...
		if ($this->request->is('post')) {
			...
			if($addOrganizationDetail){ 
				    ...
				    $cpn_currency_id_query = $this->SbsSubscriberOrganizationDetail->find('first',array('conditions'=>array('SbsSubscriberOrganizationDetail.id'=>$sbs_subscriber_organization_detail_id),'fields'=>array('SbsSubscriberOrganizationDetail.cpn_currency_id')));
				    $cpn_currency_id       = $cpn_currency_id_query['SbsSubscriberOrganizationDetail']['cpn_currency_id'];
					...
					if($addSubscriberDetail) {
						...
						$addSbsSettings    = $this->SbsSubscriberSetting->addSubscriberSettings ($sbs_subscriber_id, $cpn_currency_id);
						if ($addSbsSettings) {
							$password 		   = $this->generate_password();
							$addUser		   = $this->User->addSubscriber ($sbs_subscriber_id, $password);
							if($addUser){
								$tmplData = array();
								$tmpltArr	   = $this->SbsEmailTemplate->find('all');
							
								foreach($tmpltArr as $key=>$value){
									$tmplData['email_template_name'] 	= $value['SbsEmailTemplate']['template_name'];
									$tmplData['from_email_address'] 	= $this->Session->read('email');
                                                                        if($value['SbsEmailTemplate']['module_related'] == "Credit"){
									   $tmplData['email_subject'] = "New Credit";									       }else{
									  $tmplData['email_subject']   = $value['SbsEmailTemplate']['module_related'];
									}
									$tmplData['module_related'] 		= $value['SbsEmailTemplate']['module_related'];
									$addTemplate	   = $this->SbsEmailTemplateDetail->addTemplate ($sbs_subscriber_id, $tmplData);
								}
	...
	function paidCheckout () {		
		...
		/*SET SUCCESS AND FAIL URL*/
			...
			$root      =  $this->webroot; 
			$returnURL = "https://$host$root".'users/reviewOrder';
			$cancelURL = "https://$host$root".'users/signupFailure';
		...
		if ($this->request->is('post')) {
			...
			$L_BILLINGAGREEMENTDESCRIPTION0 = "$subscription_name : Monthly Recurring Charge (Incl. Tax) = $bill_amount $currency_code with effect from $recurrinBillDate. Initial Amt : $initial_amount $currency_code";
	...
	public function signupSuccess () {
		...
		if($this->Session->read('email')){
			...
			$sbscrncycode     = $this->Session->read('cpn_currency');			
			$this->Session->destroy();
		}
		...
		if($subscriptionType){
		   ...
		   $this->Session->write('bill_amount', $bill_amount);
		   $this->Session->write('subscriptionType', $subscriptionType);
		   $this->Session->write('cpn_currency',$sbscrncycode);
		} else {
			$profile_status   = $this->Session->read('profile_status');
			$bill_amount      = $this->Session->read('bill_amount');
			$subscriptionType = $this->Session->read('subscriptionType');
		...
		$this->set(compact('profile_status','bill_amount','subscriptionType','sbscrncycode'));	
	...
	function isCompanyExist(){
		$this->autoRender = false;	
		$cmpnyName = $this->params->query['company_name'];		
		$this->loadModel('SbsSubscriberOrganizationDetail');		 
	    	$cmpny_exist = $this->SbsSubscriberOrganizationDetail->find('first', array (
			'conditions' => array (
				'SbsSubscriberOrganizationDetail.organization_name' => $cmpnyName
			)
		));		
 		if($cmpny_exist){
 			echo "true";	
		...
		if(!empty($UserDetails['Auth']['User']['id']) && $UserDetails['Auth']['User']['active'] == 'Y') {
			...
			if($UserDetails['Auth']['User']['user_type'] == "Subscriber") {
				...
				if($plandetails['CpnSubscriptionPlan']['type'] == "Free") {
				    ...
				    if($UserDetails['Auth']['User']['SbsSubscriber']['status'] == 'Cancelled' && date('Y-m',strtotime($UserDetails['Auth']['User']['SbsSubscriber']['updation'])) < date('Y-m')) {
		...
		if ($this->request->is('post')) {
			...
			if ($this->Auth->login()) {
				...
				if(!empty($UserDetails['Auth']['User']['id'])/* && $UserDetails['Auth']['User']['active'] == 'Y'*/) {
					...
					foreach ($mennnnuuusss as $asd) {
						if($rrr == 1) {
							if(empty($asd['childMenu'])) {
								$url = $asd['Aco']['url'];
							} else {
								foreach ($asd['childMenu'] as $chileMenuUrlRedirect) {
									if($rrr == 1) {
                    ...
                    if($UserDetails['Auth']['User']['active'] == 'N') {
    ...
    public function reactivate() {
        $AuthVar = $this->Session->read('Auth');
        if(!empty($AuthVar)) {
            $this->Session->write('Upgrade.Auth',$AuthVar);
            $this->Session->delete('Auth');
            $this->loadModel('CpnSubscriptionPlan');
            $planID = $this->Session->read('Upgrade.Auth.User.SbsSubscriber.cpn_subscription_plan_id');
            $this->Session->write('Upgrade.Auth.User.SbsSubscriber.reactivate','1');
            $plan = $this->CpnSubscriptionPlan->findById($planID);
 ...
 * **/
	...
	public function createAdminUser() {
		...
		if(empty($adminUserGroupExist)) {
			...
			$adminUsreGroupID = $aro->getLastInsertID();
		}
		...
		$adminUserExist = $this->User->find('first',array('conditions'=>array('User.email'=>'admin@cantorix.com')));
		if(empty($adminUserExist)) {
			$saveUser->data = null;
			$this->User->create();			
			$saveUser->data['User']['username']  			= 'admin';
			$saveUser->data['User']['email']				= 'admin@cantorix.com';
			...
			if($this->User->save($saveUser->data)) {
				...
				$aro->create();
				$data['alias']       = 'Admin';
				$data['parent_id']   = $adminUsreGroupID;
 ...
 * **/
	public function manageUsers($user_id = null,$userName = 0, $emailId = 0, $role = 0) {
		...
		$admin_roles = $userGroup->find('list',array('conditions'=>array('Aro.parent_id'=>$parenttIDDDD['Aro']['id'],'Aro.foreign_key'=>NULL),'fields'=>array('Aro.id','Aro.alias')));
		...
		if(!empty($this->data['DeleteSelected_x'])) {
			$FORM_DATA['DELETE'] = $this->data;
			$this->deleteAllUser($FORM_DATA);
		}
		...
		if(!empty($this->data['Filter'])) {
            ...
            $joins = array(array('table'=>'aros','alias'=>'Aro','type'=>'RIGHT','conditions'=>array('Aro.foreign_key = User.id')));
			$this->Paginator->settings = array(
				'joins'=>$joins,'conditions'=> $conditions,'limit' => $limit,'order'=>array('User.id'=>'desc'),'fields'=>array('User.*','Aro.*')
			);
		}
	...
	public function deleteAllUser($data) {
		...
		$count = 0;
		foreach ($data['DELETE']['User'] as $userDetail) {
			if ($userDetail['delete'] == 1) {
				$AroIDS[$userDetail['aro_id']] = $userDetail['aro_id'];
				$userIDS[$userDetail['id']] = $userDetail['id'];
			}
		}
		$Aro = $this->Acl->Aro;
		if($subsID > 0) {
			$AroIDS1 = $Aro->find('list',array('fields'=>array('Aro.foreign_key','Aro.foreign_key'),'conditions'=>array('Aro.id'=> $AroIDS,'Aro.sbs_subscriber_id'=>$subsID)));
			$userIDS1 = $this->User->find('list',array('fields'=>array('User.id','User.id'),'conditions'=>array('User.id'=>$userIDS,'User.sbs_subscriber_id'=>$subsID)));
		} else {
			$AroIDS1 = $Aro->find('list',array('fields'=>array('Aro.foreign_key','Aro.foreign_key'),'conditions'=>array('Aro.id'=> $AroIDS,'Aro.sbs_subscriber_id'=>0)));
			$userIDS1 = $this->User->find('list',array('fields'=>array('User.id','User.id'),'conditions'=>array('User.id'=>$userIDS,'User.sbs_subscriber_id'=>0)));
		}
		$count = count($AroIDS1);
		if(!empty($AroIDS1)) {
			$userPermisions = $Aro->deleteAll(array('Aro.foreign_key'=> $AroIDS1), FALSE);
		}
		if(!empty($userIDS1)) {
			$usersDeletes = $this->User->deleteAll(array('User.id'=> $userIDS1), FALSE);
		}
		
		if($count > 1) $text = $count.' users'; else $text = $count.' user';
		if ($usersDeletes && $userPermisions) {
			$this->Session->setFlash(__('<div class="alert alert-block alert-success">'. $text.' has been deleted.</div>'));
		} elseif(empty($usersDeletes) && empty($userPermisions)) {
			$this->Session->setFlash(__('<div class="alert alert-danger">Please select atleast one user to deleted.</div>'));
		} else {
			$this->Session->setFlash(__('<div class="alert alert-danger">'.$text.' could not be deleted. Please, try again.</div>'));
		}
		if($subsID == 0) {
			$this->redirect(array('action' => 'manageUsers'));
		} else {
 ...
 * **/
	public function editUser($id=NULL, $action = NULL) {
		...
		if(empty($subsID)) {
			...
			$parenttIDDDD = $userGroup->find('first',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.foreign_key'=>NULL,'Aro.sbs_subscriber_id'=>0,'Aro.alias'=>'Admin'),'fields'=>array('Aro.id')));
			$usergroups = $userGroup->find('list',array('conditions'=>array('Aro.parent_id'=>$parenttIDDDD['Aro']['id'],'Aro.foreign_key'=>NULL),'fields'=>array('Aro.id','Aro.alias')));
		} else {
 ...
 * **/	
	public function activateSubscriber($id = NULL,$plan=0,$company=0,$subscriberName = 0,$page = 1) {
		...
		$subscriberDetails = $this->SbsSubscriber->find('first',array('conditions'=>array('SbsSubscriber.id'=>$id),
				'fields'=>array('SbsSubscriber.id','SbsSubscriber.profileId','SbsSubscriber.is_archived')));
        	
		if((!empty($subscriberDetails['SbsSubscriber']['profileId'])) && ($subscriberDetails['SbsSubscriber']['profileId'] != 'NA')) {
			/* PAYPAL API  DETAILS */
			$API_UserName 	= $this->API_UserName;
			$API_Password 	= $this->API_Password;
			$API_Signature  = $this->API_Signature;
		...
		if(!$failed) {
			if($subscriberDetails['SbsSubscriber']['is_archived'] == 'Y') {
                $restore = $this->requestAction(array('controller'=>'pages','action'=>'restoreSubscriber',$id));
                if($restore) {
                    $msg = '<div class="alert alert-block alert-success">Subscriber has been restored and activated!</div>';
                }
            }
            $data = array('id' => $id, 'status' => 'Active','updation'=>date('Y-m-d'),'is_archived'=>'N');
            $this->SbsSubscriber->save($data);
            if($this->User->updateAll(array('User.active' => "'Y'"),array('User.sbs_subscriber_id' => $id))) {
                ...
                $body1 = 'Your subscription has been activated. Please use below link to login.';
                ...
                $this->Email->subject   = 'CantoriX Subscription Activated';
                $this->Email->replyTo   = 'admin@cantorix.com';
                $this->Email->from      = 'admin@cantorix.com';
 ...
 * **/	
	public function cancelSubscriber($id = NULL,$plan=0,$company=0,$subscriberName = 0,$page = 1) {
		...
		if(!$failed) {
			...
			$this->SbsSubscriber->save($data);
			if($this->User->updateAll(array('User.active' => "'N'"),array('User.sbs_subscriber_id' => $id,'User.user_type'=>'Subscriber'))) {
                $UserDetails = $this->User->find('first', array('conditions' => array('User.sbs_subscriber_id' => $id), 'order' => array('User.id'=>'Asc')));
                if( isset($_SERVER['HTTPS'] )  && $_SERVER['HTTPS'] != 'off' ){
                    $protocol_final = 'https';
                } else {
                ...
                $body1 = 'Your subscription has been suspended. Service has been stopped by admin. Please contact cantorix admin team to activate your subscription.';
                //$body2 = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot;
                $actionToPerform = 'Contact Admin Team';
                $imgLogo = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot.'/img/logo.png';
                $this->set(compact('body1','body2','actionToPerform','imgLogo'));
                $this->Email->to        = $UserDetails['User']['email'];
                $this->Email->subject   = 'CantoriX Subscription Suspended';
                $this->Email->replyTo   = 'admin@cantorix.com';
                $this->Email->from      = 'admin@cantorix.com';
                $this->Email->template  = 'common_template';
                $this->Email->sendAs    = 'html';
 ...
 * **/	
	...
	function changeSubscriptionCheckout ($controller = NULL, $action = NULL,$typee = NULL) {
		...
		/*SET SUCCESS AND FAIL URL*/
			...
			$root      =  $this->webroot; 
			if($typee == 'upgrade') {
				$returnURL = "https://$host$root".'users/changeSubscriptionReviewOrder/1';
				$cancelURL = "https://$host$root".'subscribers/signupFailure';
			} else {
				$returnURL = "https://$host$root".'users/changeSubscriptionReviewOrder';
				$cancelURL = "https://$host$root".'subscribers/signupFailure';
			}
	...
	public function changeSubscriptionReviewOrder ($upgrade = NULL) {
		...
		$task = "getExpressCheckout"; //set initial task as Express Checkout			
		$httpParsedResponseAr = $obj->getExpressCheckout();
		$this->set(compact('upgrade','subscriberId'));
		if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {			
			$this->set(compact('httpParsedResponseAr'));
		} else  {
		...
		if ($this->request->is('post')) {
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {		
				...
				if($upgradee) {
                    ...
                    $downgradedPlanId = $this->Session->read('Auth.User.NewPlanId');
                    $this->SbsDowngradeRequest->updateAll(array('SbsDowngradeRequest.status' => "'Inactive'"),array('SbsDowngradeRequest.sbs_subscriber_id' => $subscriberId,'SbsDowngradeRequest.status'=>'Active'));
                    $this->SbsDowngradeRequest->create();
                    $downgradeRequested = $this->SbsDowngradeRequest->save(array('sbs_subscriber_id'=>$subscriberId, 'cpn_subscription_plan_id'=>$downgradedPlanId, 'date_updated'=>date('Y-m-d'), 'status'=>'Active'));
                    if($downgradeRequested) {
                        $this->loadModel('User');
                        $UserDetails = $this->User->find('first', array('conditions' => array('User.sbs_subscriber_id' => $subscriberId), 'order' => array('User.id'=>'Asc')));
                        if( isset($_SERVER['HTTPS'] )  && $_SERVER['HTTPS'] != 'off' ){
                            $protocol_final = 'https';
                        } else {
                            $protocol_final = 'http';
                        }
                        $body1 = 'Your subscription has been downgraded to '.$subscription_name.' plan. Your downgraded plan will effect from next billing period. Kindly use the below link for login.';
                        $body2 = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot;
                        $actionToPerform = 'Login';
                        $imgLogo = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot.'/img/logo.png';
                        $this->set(compact('body1','body2','actionToPerform','imgLogo'));
                        $this->Email->to        = $UserDetails['User']['email'];
                        $this->Email->subject   = 'CantoriX Downgrade Request';
                        $this->Email->replyTo   = 'admin@cantorix.com';
                        $this->Email->from      = 'admin@cantorix.com';
                        $this->Email->template  = 'common_template';
                        $this->Email->sendAs    = 'html';
				...
				if($updateSubscriber) { 
					// signupSuccess
					if($upgradee) {
					   $newPlanId = $this->Session->read('cpn_subscription_plan_id');
					   $this->Session->write('Auth.User.SbsSubscriber.cpn_subscription_plan_id',$newPlanId);
                        ...
                        $body1 = 'Your subscription has been upgraded to '.$subscription_name.' plan. Kindly use the below link for login.';
                        $body2 = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot;
                        $actionToPerform = 'Login';
                        $imgLogo = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot.'/img/logo.png';
                        $this->set(compact('body1','body2','actionToPerform','imgLogo'));
                        $this->Email->to        = $UserDetails['User']['email'];
                        $this->Email->subject   = 'CantoriX Upgrade Subscription';
                        $this->Email->replyTo   = 'admin@cantorix.com';
                        $this->Email->from      = 'admin@cantorix.com';
                        $this->Email->template  = 'common_template';
                        $this->Email->sendAs    = 'html';
	...
	public function changeSubscriptionSignupSuccess () {		
        ...
        $currency_code      = $cpnSetting['CpnSetting']['currency_code'];
		$this->set(compact('menuActive','settingsActive'));
		$this->set(compact('profile_status','bill_amount','currency_code'));
	}
	...
	public function paymentDetailRenewal ($cpn_subscription_plan_id = null) {
		$temp = base64_decode($cpn_subscription_plan_id);
        $tempExplode = explode('-', $temp);
        $cpn_subscription_plan_id = $tempExplode[1];
		$this->loadModel('CpnSubscriptionPlan');
		$this->loadModel('CpnSetting');
	...
	function paidCheckoutRenewal () {		
		...
		/*SET SUCCESS AND FAIL URL*/
		    $host 	   =  $_SERVER['SERVER_NAME'];		    
			$root      =  $this->webroot; 
			$returnURL = "https://$host$root".'users/reviewOrderRenewal';
			$cancelURL = "https://$host$root".'users/signupFailure';
	...
	function makePaymentRenewal () {
		...
		if ($this->request->is('post')) {
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {		
				...
				$updateSubscriber = $this->SbsSubscriber->save($updateSubscriberDetail);
				if($updateSubscriber) {
				    $subscriberDetails = $this->SbsSubscriber->findById($subscriberId);
                    if($subscriberDetails['SbsSubscriber']['is_archived'] == 'Y') {
                        $restoreData = $this->requestAction(array('controller'=>'pages','action'=>'restoreSubscriber',$subscriberId));
                    }
                    $sendEmailReactivate = $this->Session->read('Upgrade.Auth.User.SbsSubscriber.reactivate');
                    $UserDetails = $this->User->find('first', array('conditions' => array('User.sbs_subscriber_id' => $subscriberId), 'order' => array('User.id'=>'Asc')));
                    if( isset($_SERVER['HTTPS'] )  && $_SERVER['HTTPS'] != 'off' ){
                        $protocol_final = 'https';
                    } else {
                    ...
                    if($sendEmailReactivate == 1) {
                        $body1 = 'Your subscription has been reactivated. Please login with the below link';
                        $body2 = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot."/";
                        $actionToPerform = 'Login';
                        $imgLogo = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot.'/img/logo.png';
                        $this->set(compact('body1','body2','actionToPerform','imgLogo'));     
                        $this->Email->to        = $UserDetails['User']['email'];
                        $this->Email->subject   = 'Cantorix Account Re-activation';
                        $this->Email->replyTo   = 'admin@cantorix.com';
                        $this->Email->from      = 'admin@cantorix.com';
                        $this->Email->template  = 'common_template';
                        $this->Email->sendAs    = 'html' ;
                        ...
                        $body1 = 'Your subscription has been upgraded. Please login with the below link';
                        $body2 = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot."/";
                        $actionToPerform = 'Login';
                        $imgLogo = $protocol_final.'://'.$_SERVER['HTTP_HOST'].$this->webroot.'/img/logo.png';
                        $this->set(compact('body1','body2','actionToPerform','imgLogo'));
                        $this->Email->to        = $UserDetails['User']['email'];
                        $this->Email->subject   = 'Cantorix Upgrade';
                        $this->Email->replyTo   = 'admin@cantorix.com';
                        $this->Email->from      = 'admin@cantorix.com';
                        $this->Email->template  = 'common_template';
                        $this->Email->sendAs    = 'html' ;
	...
	public function dashboard() {
		...
		$row1Left['totalInvoices'] 		= $this->AcrClientInvoice->find('first',array('fields'=>array('SUM(func_currency_total) as total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'invoiced_date LIKE'=>$currentMonth.'%','NOT'=>array('status'=>array('Canceled','Draft')))));
		//$exchangeRates[0]				= $this->AcrClientInvoice->find('list',array('fields'=>array('id','exchange_rate'),'conditions'=>array('sbs_subscriber_id'=>$subscriberID,'invoiced_date LIKE'=>$currentMonth.'%','NOT'=>array('status'=>array('Canceled','Draft')))));
		//$row1Left['totalPayments']		= $this->AcrInvoicePaymentDetail->getPayments($exchangeRates);
        $paymentsMadeCurrentMonth 		= $this->AcrInvoicePaymentDetail->find('all',array('fields'=>array('SUM(paid_amount) as paidAmount','acr_client_invoice_id','AcrClientInvoice.exchange_rate'),'joins'=>array(array('table'=>'acr_client_invoices','alias'=>'AcrClientInvoice','type'=>'LEFT','conditions'=>array('AcrInvoicePaymentDetail.acr_client_invoice_id = AcrClientInvoice.id'))),'conditions'=>array('AcrInvoicePaymentDetail.sbs_subscriber_id'=>$subscriberID,'is_deleted'=>'no','payment_date LIKE'=>$currentMonth.'%'),'group'=>array('acr_client_invoice_id')));
        
        foreach ($paymentsMadeCurrentMonth as $paymentMonth) {
			$paidAmountMonth		+= ($paymentMonth[0]['paidAmount'] * $paymentMonth['AcrClientInvoice']['exchange_rate']);
		}
		$row1Left['totalPayments'] = $paidAmountMonth;
		
		$row1Left['totalExpenses'] 		=  $this->AcpExpense->find('first',array('fields'=>array('SUM(amount) as total'),'conditions'=>array('sbs_subscriber_id'=>$subscriberID,'date LIKE'=>$currentMonth.'%')));
		...
		$recievableInvoices					= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')))));
		...
		$recievableOverDueInvoices			= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date <'=>date('Y-m-d'))));
		...
		$recievableDueInvoices				= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date >='=>date('Y-m-d'))));
		...
		/*******************************Unpaid Invoices**************************************/
			$recievableCurrentDueInvoices		= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date >'=>date('Y-m-d'))));
			...
			$currentDueInvoices					= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date'=>date('Y-m-d'))));
			$currentDueAmount = 0;
			...
			$agingBuckets				= $this->SbsAgingBucket->find('all',array('conditions'=>array('sbs_subscriber_id'=>$subscriberID),'order'=>array('id'=>'Asc'),'fields'=>array('id','bucket','from_day','to_day')));
            if(empty($agingBuckets)) {
                $agingBuckets[0]['SbsAgingBucket']['from_day']          = 0;
                $agingBuckets[0]['SbsAgingBucket']['to_day']            = 30;
                $agingBuckets[0]['SbsAgingBucket']['bucket']            = '0-30 days';
                $agingBuckets[0]['SbsAgingBucket']['sbs_subscriber_id'] = $subscriberID;
                
                $agingBuckets[1]['SbsAgingBucket']['from_day']          = 31;
                $agingBuckets[1]['SbsAgingBucket']['to_day']            = 60;
                $agingBuckets[1]['SbsAgingBucket']['bucket']            = '31-60 days';
                $agingBuckets[1]['SbsAgingBucket']['sbs_subscriber_id'] = $subscriberID;
                
                
                $agingBuckets[2]['SbsAgingBucket']['from_day']          = 61;
                $agingBuckets[2]['SbsAgingBucket']['to_day']            = 90;
                $agingBuckets[2]['SbsAgingBucket']['bucket']            = '61-90 days';
                $agingBuckets[2]['SbsAgingBucket']['sbs_subscriber_id'] = $subscriberID;
                
                $agingBuckets[3]['SbsAgingBucket']['from_day']          = 91;
                $agingBuckets[3]['SbsAgingBucket']['to_day'];
                $agingBuckets[3]['SbsAgingBucket']['bucket']            = '90+ days';
                $agingBuckets[3]['SbsAgingBucket']['sbs_subscriber_id'] = $subscriberID;
                
            }
			if(!empty($agingBuckets)) {
				foreach ($agingBuckets as $icount => $bucket) {
					if($icount == 0 && $bucket['SbsAgingBucket']['from_day'] == 0) {
						$from[$icount]					= date('Y-m-d', strtotime('today - '.$bucket['SbsAgingBucket']['to_day'].' days'));
						$to[$icount] 				    = date('Y-m-d',strtotime("-1 days"));
					} else {
						if(!empty($bucket['SbsAgingBucket']['to_day'])) {
							$from[$icount]				= date('Y-m-d', strtotime('today - '.$bucket['SbsAgingBucket']['to_day'].' days'));
							$difference[$icount] 		= ($bucket['SbsAgingBucket']['to_day'] - $bucket['SbsAgingBucket']['from_day']);
							$to[$icount]				= date('Y-m-d', strtotime($from[$icount].'+'.$difference[$icount].' days'));
						} else {
							$from[$icount]				= date('Y-m-d', strtotime('today - '.$bucket['SbsAgingBucket']['from_day'].' days'));
						}
					}
					
					if(!empty($bucket['SbsAgingBucket']['to_day'])) {
						$agingBucketsInv[$icount]				= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date BETWEEN ? AND ? '=>array($from[$icount],$to[$icount]))));
					} else {
						$agingBucketsInv[$icount]				= $this->AcrClientInvoice->find('all',array('fields'=>array('id','exchange_rate','invoice_total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft','Marked as paid')),'due_date <= '=>$from[$icount])));
					}
				...
				$row3Right['agingBucketsLabel']			= $agingBuckets;
			}
	...
	public function dashboardGraph() {
		...
		foreach ($period['period'] as $dt) {
			...
			$invoiceArray[] 	= $this->AcrClientInvoice->find('first',array('fields'=>array('SUM(func_currency_total) as total'),'conditions'=>array('AcrClientInvoice.recurring'=>'N','sbs_subscriber_id'=>$subscriberID,'NOT'=>array('status'=>array('Canceled','Draft')),'invoiced_date LIKE'=>$dt->format('Y-m').'%')));
			$paidAmount = 0;
			$paymentsMade       = $this->AcrInvoicePaymentDetail->find('all',array('fields'=>array('SUM(paid_amount) as paidAmount','acr_client_invoice_id','AcrClientInvoice.exchange_rate'),'joins'=>array(array('table'=>'acr_client_invoices','alias'=>'AcrClientInvoice','type'=>'LEFT','conditions'=>array('AcrInvoicePaymentDetail.acr_client_invoice_id = AcrClientInvoice.id'))),'conditions'=>array('AcrInvoicePaymentDetail.sbs_subscriber_id'=>$subscriberID,'is_deleted'=>'no','payment_date LIKE'=>$dt->format('Y-m').'%'),'group'=>array('acr_client_invoice_id')));
			foreach ($paymentsMade as $payment) {
			    $paidAmount		+= ($payment[0]['paidAmount'] * $payment['AcrClientInvoice']['exchange_rate']);
			}
			$paymentArray[] = $paidAmount;
			$expenseArray[] = $this->AcpExpense->find('first',array('fields'=>array('SUM(amount) as total'),'conditions'=>array('sbs_subscriber_id'=>$subscriberID,'date LIKE'=>$dt->format('Y-m').'%')));
		...
		$final['expenseArray']			= $expenseArray;
	...
	public function dashboardCustomerGraph() {
		...
		$customerInvoice 		= $this->AcrClientInvoice->find('all',array(
				'fields'=>array('SUM(func_currency_total) as total','AcrClientInvoice.acr_client_id','AcrClient.organization_name'),
				'conditions'=>array('AcrClientInvoice.recurring'=>'N','AcrClientInvoice.sbs_subscriber_id'=>$subscriberID,'NOT'=>array('AcrClientInvoice.status'=>array('Canceled','Draft')),'AcrClientInvoice.invoiced_date BETWEEN ? AND ?'=>array($fromDate,$toDate)),
	...
	public function dashboardExpenseGraph() {
		$subscriberID = $this->Session->read('Auth.User.SbsSubscriber.id');
		if(empty($this->data['Dashboard']['expenseGraph'])) {
			$this->request->data['Dashboard']['expenseGraph'] = 'Current Month';
		}
		$period = $this->getPeriod($this->data['Dashboard']['expenseGraph']);
		$this->loadModel('AcpExpense');
		$i = 1;
		foreach ($period['period'] as $dt) {
			if($i==1) $fromDate	= $dt->format('Y-m').'-01';
        ...
        $this->AcpExpense->unbindModel(array('belongsTo'=>array('AcpExpenseCategory','AcrClient','SbsSubscriber')));
		$expenses 		= $this->AcpExpense->find('all',array('fields'=>array('amount','AcpVendor.vendor_name'),'conditions'=>array('AcpExpense.sbs_subscriber_id'=>$subscriberID,'AcpExpense.date BETWEEN ? AND ?'=>array($fromDate,$toDate)),
				'order' => array('amount'=>'Desc'),'limit'	=> 10
		));
	...
	public function adminDashboard() {
		...
		$dashboardActive = 'active';
		$this->set(compact('allPlan','dashboardActive','title_for_layout'));
		foreach($allPlan as $planId => $planName){
			...
			if($getsubscriber){
                ...
                $getLastMonthSubscriptionCount['Paid Subscription'] = 0;
                $getLastMonthSubscriptionCount['Due Subscription']  = 0;
                foreach ($getsubscriberLastMonth as $subscriberLastMonth) {
                    
                    $getLastMonthSubscriptionStatus = $this->CpnSubscriberInvoiceDetail->getLastMonthInvoiceStatusBasedOnSubscriberId($subscriberLastMonth['SbsSubscriber']['id']);
                    ...
                    } elseif($subscriberLastMonth['SbsSubscriber']['status'] == 'Cancelled' && date('Y-m',strtotime($subscriberLastMonth['SbsSubscriber']['updation'])) == date('Y-m',strtotime("-1 month"))) {
                        $getLastMonthSubscriptionCount['Cancel Subscription'] = $lastMonthCancelledCount++;
                    }
                }
                
                $currentCancelledSubscriptionCount = $this->SbsSubscriber->find('count',array('conditions'=>array('SbsSubscriber.status'=>'Cancelled','updation LIKE'=>date('Y-m').'%')));
                $previousCancelledSubscriptionCount = $this->SbsSubscriber->find('count',array('conditions'=>array('SbsSubscriber.status'=>'Cancelled','updation LIKE'=>date('Y-m',strtotime("-1 month")).'%')));
                $getSubscriptionCount['Cancel Subscription']    = $currentCancelledSubscriptionCount;
				...
				$growthPercentageOther['Cancel Subscription'] 	= $this->calculateGrowth($currentCancelledSubscriptionCount,$previousCancelledSubscriptionCount);
			}else{
                ...
                if($getsubscriberLastMonth){
                    $LastMonthCount = count($getsubscriberLastMonth);
                } else {
                    $LastMonthCount = 0;
                }
                $growthPercentage = $this->calculateGrowth($getSubscriptionCount[$planName],$LastMonthCount);
                $getSubscriptionGrowth[$planName]   =   $growthPercentage;
        ...
        $growthPercentageOther['Paid Subscription']     = $this->calculateGrowth($getRevenueDetails[date("Y-M",strtotime(date( 'Y-m-01' )))]['Subscription'],$getRevenueDetails[date("Y-M",strtotime( date( 'Y-m-01' )." -1 months"))]['Subscription']);
        $growthPercentageOther['Due Subscription']      = $this->calculateGrowth($getRevenueDetails[date("Y-M",strtotime(date( 'Y-m-01' )))]['Due Subscription'],$getRevenueDetails[date("Y-M",strtotime( date( 'Y-m-01' )." -1 months"))]['Due Subscription']);
	...
	public function calculateGrowth($currentMonthSubscription,$previousMonthSubscription){
		if($currentMonthSubscription>$previousMonthSubscription){
            ...
            /*Saurabh Calculations*/
			     //$value = (($previousMonthSubscription - $currentMonthSubscription)/$currentMonthSubscription)*100;
            /*End Saurabh Calculations*/
            
            /*Ganesh Calculations as per freddie formula to calculate loss percerntage*/
            if($previousMonthSubscription>0){
                $value = (($currentMonthSubscription-$previousMonthSubscription)/$previousMonthSubscription)*100;
            } else {
                $value = round($currentMonthSubscription)*100;
            }
            /*End Ganesh Calculations as per freddie formula to calculate loss percerntage*/
			$arrayValue['val']	=	round($value);
		}else{
    ...
    /*public function calculateGrowthCancel($currentMonthSubscription,$previousMonthSubscription){
        if($currentMonthSubscription > $previousMonthSubscription) {
            $arrayValue['Growth'] = "down";
            $value = (($previousMonthSubscription - $currentMonthSubscription)/$currentMonthSubscription)*100;
            $arrayValue['val']  =   round($value);
