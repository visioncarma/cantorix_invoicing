 ...
 */
	class CustomersController extends AppController {
		...
		public function index($organization = 0,$client = 0,$email = 0,$status = 0,$page=1) {
			...
			if(!$limit) $limit = 10;
			$this->Paginator->settings = array('joins'=>array(array('table'=>'acr_client_contacts','alias'=>'AcrClientContact','type'=>'LEFT','conditions'=>array('AcrClientContact.acr_client_id = AcrClient.id','AcrClientContact.is_primary'=>'Y'))),'conditions'=>$conditions,'limit'=>$limit,'order'=>$order,
				'fields'=>array('AcrClient.id','AcrClient.client_no','AcrClient.client_name','AcrClient.organization_name','AcrClient.business_phone','AcrClient.status','AcrClient.sbs_subscriber_id','AcrClientContact.id','AcrClientContact.email','AcrClientContact.name','AcrClientContact.sur_name'));
			$customers = $this->Paginator->paginate('AcrClient');
			$this->set('clients',$customers);
		...
		public function deleteAllCustomers() {
			...
			$this->loadModel('AcrClientCustomValue');
			$this->loadModel('AcrClientContact');
			foreach ($this->data['Client']['id'] as $customerID => $delete) {
				if($delete) {
					$customerIDS[$customerID] = $customerID;
				}
			}
			$finalCustomerIDS = $this->AcrClient->find('list',array('conditions'=>array('AcrClient.id'=>$customerIDS,'AcrClient.sbs_subscriber_id'=>$this->subscriber)));
			$invoice = $this->AcrClientInvoice->find('list',array('conditions'=>array('AcrClientInvoice.acr_client_id'=>$finalCustomerIDS),'fields'=>array('acr_client_id','acr_client_id')));
			$quote = $this->SlsQuotation->find('list',array('conditions'=>array('SlsQuotation.acr_client_id'=>$finalCustomerIDS),'fields'=>array('acr_client_id','acr_client_id')));
			$InactiveCustomers = $invoice + $quote;
			$deleteCustomers = array_diff_key($customerIDS,$InactiveCustomers);
			$deleteCustCount = count($deleteCustomers);
			$InactiveCustCount = count($InactiveCustomers);
			$inactiveMsg = NULL;$deleteMsg=NULL;
			if($deleteCustCount > 0) {
				$this->AcrClientCustomValue->deleteAll(array('AcrClientCustomValue.acr_client_id'=>$deleteCustomers),FALSE);
				$this->AcrClientContact->deleteAll(array('AcrClientContact.acr_client_id'=>$deleteCustomers),FALSE);
				$deleteed = $this->AcrClient->deleteAll(array('AcrClient.id'=>$deleteCustomers),FALSE);
				if ($deleteed) {
					$deleteMsg = $deleteCustCount.' customers has been deleted';
				}
			} 
			if($InactiveCustCount > 0) {
				
				$updated = $this->AcrClient->updateAll(
						array('AcrClient.status' => "'Inactive'"),array('AcrClient.id ' => $InactiveCustomers)
				);
				if($updated) {
					$inactiveMsg = $InactiveCustCount.' customers has been made inactive due to existing invoices/quotes';
				}
			}
			if(!empty($inactiveMsg) && !empty($deleteMsg)) {
				$finalMsg = '<div class="alert alert-block alert-success">'.$deleteMsg.' and '.$inactiveMsg.'</div>';
			} elseif(!empty($inactiveMsg) && empty($deleteMsg)) {
				$finalMsg = '<div class="alert alert-block alert-success">'.$inactiveMsg.'</div>';
			} elseif(empty($inactiveMsg) && !empty($deleteMsg)) {
				$finalMsg = '<div class="alert alert-block alert-success">'.$deleteMsg.'</div>';
			} else {
				$finalMsg = '<div class="alert alert-danger">Internal Error occurred. Try agian later.</div>';
			}
			$this->Session->setFlash($finalMsg);
		...
		public function add() {
			...
			if($this->data){
				...
				if(empty($this->data['AcrClient']['cpn_currency_id'])){
				    $this->request->data['AcrClient']['cpn_currency_id'] = $defaultCurrenyId;
				}else{
					$this->request->data['AcrClient']['cpn_currency_id'] = $this->data['AcrClient']['cpn_currency_id'];
				}
                if($this->data['AcrClient']['shipping_address']){
                	$this->request->data['AcrClient']['shipping_country']= $this->data['AcrClient']['billing_country'];
                }
		...
		public function edit($id = NULL,$subscriberID = NULL,$org = 0,$client = 0,$eml = 0,$st = 0,$page=1){ 
            ...
            if(!$this->AcrClient->_checkFraud($id)) {
			...
			if($this->data){
				    ...
				    if(empty($client_contact_details) && empty($this->data['Add']['Contact1']['name'])){
				    	  $this->Session->setFlash(('<div class="alert alert-danger">'.__('Please add atleast one contact').'</div>'));
				          $this->redirect(array('action'=>'edit',$id,$subscriberID,$org,$client,$eml,$st,$page));
					}else{
				         ...
				         $this->AcrClient->save($addClientName,array('validate'=>false));
				  		 ...
				  		 if($this->data['FieldValue']){
				  		  	  foreach($this->data['FieldValue'] as $key1=>$val1){
				  		  	  	       if($val1){
				  		  	  	       	     $custom_valId= $this->AcrClientCustomValue->find('first',array('conditions'=>array('AcrClientCustomValue.acr_client_custom_field_id'=>$key1,'AcrClientCustomValue.acr_client_id'=>$id)));
											 
				  		  	  	       		 $editFieldValue=null;
				  		  	          	     $editFieldValue['AcrClientCustomValue']['id']   = $custom_valId['AcrClientCustomValue']['id']; 
				  		  	           	     $editFieldValue['AcrClientCustomValue']['data'] = $val1;
				  		  	          	     $this->AcrClientCustomValue->save($editFieldValue,array('validate'=>false));
				  		  	  	       }else{
				  		  	  	       	    $custom_valId= $this->AcrClientCustomValue->find('first',array('conditions'=>array('AcrClientCustomValue.acr_client_custom_field_id'=>$key1,'AcrClientCustomValue.acr_client_id'=>$id)));
				  		  	  	       	    $this->AcrClientCustomValue->delete($custom_valId['AcrClientCustomValue']['id']);
				  		  	  	       }
				  		  	  	       
				  		  	  }
				  		}
                        if($this->data['AddFieldValue']){
                        	 foreach($this->data['AddFieldValue'] as $key2=>$val2){
					        	
					        	  $addFieldValue=null;
					        	  $this->AcrClientCustomValue->create();
					        	  $addFieldValue['AcrClientCustomValue']['data']                       = $val2;
					        	  $addFieldValue['AcrClientCustomValue']['acr_client_id']              = $id;
					        	  $addFieldValue['AcrClientCustomValue']['acr_client_custom_field_id'] = $key2;
					        	  $this->AcrClientCustomValue->save($addFieldValue);
					       }
                        }
                        
						$this->Session->setFlash(__('<div class="alert alert-block alert-success">Customer Details has been updated successfully.</div>'));
		...
		public function view($id = NULL,$subscriberID = NULL, $org = 0,$client = 0,$eml = 0,$st = 0,$page=1){
			...
			$clientInvoice= $this->AcrClientInvoice->find('list',array(
				'conditions'=>array('AcrClientInvoice.acr_client_id'=>$id,'NOT' => array('AcrClientInvoice.status'=>array('Canceled','Draft','Paid','Marked as paid'))),
				'fields'=>array('AcrClientInvoice.id','AcrClientInvoice.id')
			));
			$totalInvoiceAmount = $this->AcrClientInvoice->find('first',array(
				'conditions'=>array('AcrClientInvoice.acr_client_id'=>$id,'NOT' => array('AcrClientInvoice.status'=>array('Canceled','Draft','Paid','Marked as paid'))
				),
				'fields'=>array('AcrClientInvoice.id','SUM(AcrClientInvoice.invoice_total) as invoiceAmount')
			));
			$this->loadModel('AcrInvoicePaymentDetail');
			$paidAmount = $this->AcrInvoicePaymentDetail->find('first',array('fields'=>array('SUM(paid_amount) as paidAmount'),'conditions'=>array('AcrInvoicePaymentDetail.acr_client_invoice_id'=>$clientInvoice,'is_deleted'=>'no')));
			$amount_due=$totalInvoiceAmount[0]['invoiceAmount'] - $paidAmount[0]['paidAmount'];
 ...
 */
		function show_excel() {
			...
			if($this->data){
										...
										if($excel->boundsheets[$i]['name'] == 'Clients Informations'){
											...
											foreach($clientInformation as $key=>$informations){
												...
												if(empty($orgExists)){
													...
													if($getLanguageId){
														$getPaymentTermId = $this->SbsSubscriberPaymentTerm->getTermsByName(trim($informations['Payment Terms *']),$subscriberId);
														if($getPaymentTermId){
															$isCurrencyExist = $this->CpnCurrency->isCurrencyExist($informations['Currency Code *']);
															if($isCurrencyExist){
																$getCurrencyId = $this->SbsSubscriberCpnCurrencyMapping->isCurrencyExistForSubscriber($isCurrencyExist,$this->subscriber);
															}
															if($getCurrencyId){
																if($contactArray[$key]['Client Sl No. *'] == $informations['Client Sl No. *']){
																	$newClientId = $this->AcrClient->importClient($subscriberId,$informations,$getLanguageId,$isCurrencyExist,$getPaymentTermId);
																	if($newClientId){
																		...
																		if($NewClientContact && $newClientId['Success']){
																			if($NewClientContact['Success']){
																				/*$clientContactInformation = $NewClientContact['contactSuccess'];*/
																	...
																	$errorMessage[$key]['Client Sl No'] 		=  $informations['Client Sl No. *'];
																	$errorMessage[$key]['Organization Name'] 	=  $informations['Organization Name *'];
																	$errorMessage[$key]['Error Message'] 		=  "Contact information missing";
																}
															...
															$errorMessage[$key]['Client Sl No'] 		=  $informations['Client Sl No. *'];
															$errorMessage[$key]['Organization Name'] 	=  $informations['Organization Name *'];
															$errorMessage[$key]['Error Message'] 		=  "Payment Term is not supported";
														}
				...
				$noofcustomers = $this->CpnSubscriptionPlan->find('first',array('conditions'=>array('CpnSubscriptionPlan.id'=>$cpn_subscription_plan_id),'fields'=>array('CpnSubscriptionPlan.no_of_clients')));
				$presentCustCount = $this->AcrClient->find('count',array('conditions'=>array('AcrClient.sbs_subscriber_id'=>$this->subscriber,'AcrClient.status'=>'active')));
				if(($noofcustomers['CpnSubscriptionPlan']['no_of_clients'] > $presentCustCount) || ($noofcustomers['CpnSubscriptionPlan']['no_of_clients'] == '-1')) {
					$showAddButton = TRUE;
				} else {
					$showAddButton = FALSE;
