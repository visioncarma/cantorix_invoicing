 ...
 */
	...
	public function add_group(){
		...
		if($this->request->is('post')) {
			...
			if(empty($this->data['SbsSubscriberTaxGroup']['group_name'])){
				 $this->Session->setFlash('<div class="alert alert-danger">Group name should not be empty.</div>');
			     $this->redirect(array('controller'=>'tax_groups','action' => 'index'));
			
			}elseif($taxGroupExist){
	...
	public function edit_group($id=null){
		...
		if($this->data['SbsSubscriberTaxGroup']['group'.$id]) {
	...
	public function groupcheckempty(){
		$this->autoRender=false;
		if(!empty($this->params->query['group_name'])){
	    	echo 1;
	    }else{
	    	echo 0;
	    }
