...
<div class="page-content">
			<div class="page-header">
				...
				<div class="headernew col-lg-4 col-md-5 col-sm-4 col-xs-4 width-after-600"> User Management </div>
				...
				<div class="col-lg-8 col-md-7 col-sm-8 col-xs-8 no-padding-left no-padding-right width-after-600">
					<div class="widthauto paddingleftrightzero pull-right buttonrightwidth padding-right-3-480 ">
						<button class="btn btn-sm btn-success pull-right addbutton width-after-400" data-toggle="modal" data-target="#addnewuser">
							<i class="icon-plus"></i>
							Add New User
						</button>
					</div>
		...
		<div class="col-xs-12">
			<div class="table-responsive table-responsivenoscroll tablehidemobilee">
				...
				<div class="row margin-twenty-zero filterdivmob">
					...
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 nopadding  col-sm-3  col-lg-2 choosen_width">
						<?php echo $this->Form->input('role',array('class'=>'form-control invdrop','data-placeholder'=>'Select Role','options'=>array(''=>'Select Role',$usergroups)));?>	
                    </div>
					<div class="form-group filed-left margin-bottom-zero mobile_100">
						<?php echo $this->Form->submit('Filter',array('url'=>array('controller'=>'users','action'=>'index'),'class'=>'btn btn-sm btn-primary filter-btn form-control mobile_100','type'=>'submit'));?>
					</div>
					<div class="form-group filed-left margin-bottom-zero resetbutton mobile_100">
						<?php echo $this->Html->link('Reset',array('controller'=>'users','action'=>'index'),array('class'=>'btn btn-sm btn-primary filter-btn form-control mobile_100','title'=>'Reset filtered result'));?>
					</div>
				...
				<?php echo $this->Form->create('User',array('url'=>array('controller'=>'users','action'=>'index'),'id'=>'User','inputDefaults'=>array('div'=>FALSE,'label'=>FALSE,'class'=>'on-edit')));?>
                 ...
                 <?php
					if($permission['_delete'] == '1') {
						echo $this->Form->submit('delete_selected.png',array('url'=>array('action'=>'index'),'class'=>'deleteicon delete','type'=>'submit','title'=>'Delete Selected', 'name' => 'DeleteSelected', 'value' => 'DeleteSelected','escape'=>FALSE,'onclick'=>"return confirm('Are you sure you want to delete selected users ?')", 'update'=>'#content'));
					}
				?>
                </div>
				<table id="sample-table-1" class="table table-striped table-bordered table-hover editable-table">
					<thead>
						<tr>
							...
							<th class="width120"><?php echo $this->Paginator->sort('username','User Name',array('url'=>$url,'lock'=>TRUE));?></th>
							<th class="width120"><?php echo $this->Paginator->sort('email','Email',array('url'=>$url,'lock'=>TRUE));?></th>
							<th class="width120">Role</th>
							<th class="width120"><?php echo $this->Paginator->sort('active','Active',array('url'=>$url,'lock'=>TRUE));?></th>
					...
					<tbody>
						...
						<td class="class="width10"">
							<span class=""> 
								<label>
									<?php if($permission['_delete'] == '1' && $adminUser['User']['id'] != $user['User']['id']):?>
									<?php echo $this->Form->checkbox($user['User']['id'].'.delete',array('class'=>'ace delete-m-row'));?>
									<?php endif;?>
									<span class="lbl"></span> 
								</label> 
							</span>
						...
						<td class="width120">
							...
							<?php if($permission['_update'] == '1'):?>
							<span class="on-edit inlineeditselectpick inlinerole">	<?php echo $this->Form->input($user['User']['id'].'.aro_id',array('class'=>'selectpicker ','options'=>array($usergroups),'placeholder'=>'Role','data-placeholder'=>'Role','default'=>$user['usergroup_details']['Aro']['id']));?></span>
							<?php endif;?>
						</td>
						<td class="width120">
							...
							<?php if(($permission['_update'] == '1' && $showAddButton) || $planDetails['CpnSubscriptionPlan']['no_of_staffs'] == count($users)):?>
								<span class="on-edit inlineeditselectpick inlineedit"> <?php echo $this->Form->input($user['User']['id'].'.active',array('class'=>'selectpicker','data-placeholder'=>'Active','options'=>array('Y'=>'Yes','N'=>'No'),'default'=>$user['User']['active']));?> </span>
							<?php elseif($permission['_update'] == '1' && !$showAddButton):?>
								<span class="on-edit inlineeditselectpick inlineedit"> <?php echo $this->Form->input($user['User']['id'].'.active',array('class'=>'selectpicker','data-placeholder'=>'Active','options'=>array('Y'=>'Yes','N'=>'No'),'default'=>$user['User']['active'],'disabled'=>TRUE));?> </span>
							<?php endif;?>
						</td>
						<td class="width120">
								<div class="visible-md visible-lg visible-sm hidden-xs btn-group">
									...
									<a class="btn btn-xs btn-success view on-load" data-toggle="modal" data-target="#viewuser<?php echo $user['User']['id'];?>">
										<i class="icon-zoom-in bigger-120"></i>
									</a>
			...
			<!-- Only mobile -->
						...
						<div class="table-small-view wordwrap">
                          ...
                          <?php foreach($users as $user){?>
                          <div class="row marginleftrightzero nopaddingleft nopaddingright borderfull paddingtopbottom5 contentrow borderbottom mobileUserView">
                             <div class="col-xs-12 marginleftrightzero nopaddingleft nopaddingright paddingtopbottom5">
                                <div class="col-xs-6">
                                     <span class="pull-left">
                                        <label>
						...
						<!--end of pop-->
                                    	<!--<div class="inline position-relative">
                                    		...
                                    		<button class="btn btn-minier btn-info dropdown-toggle mainbtn" data-toggle="dropdown">
											...
											<ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
												<li>
													...
													<div class="btn btn-xs btn-success view on-load" title="View" data-toggle="modal" data-target="#viewusermobile<?php echo $user['User']['id'];?>">
														<i class="icon-zoom-in bigger-120"></i>
													</div>											
												</li>
												...
												<li>
													<?php if($permission['_update'] == '1' && $adminUser['User']['id'] !=$user['User']['id']):?>
													<a class="btn btn-xs btn-info edit edit-row on-load padding3" title="Edit">
														<i class="icon-edit bigger-120"></i>
													</a>
													<?php endif;?>
												</li>
												...
												<li>
													...
													<?php echo $this->Html->link('<i class="icon-trash bigger-120"></i>',
																array('action' => 'delete', $user['User']['id'],$subscriberID), array('class'=>'btn btn-xs btn-danger delete on-load padding3','title'=>'Delete','escape'=>FALSE), __('Are you sure want to delete user ?')); ?>
													<?php endif;?>
												</li>
											</ul>
                             ...
                             <div class="col-xs-4 pull-right">
								<a href="javascript:void(0)" class="save-row on-edit" style="display:none;">
									<?php echo $this->Js->submit('', array('div'=>false,'class'=>'save-row submit greennn bgwhitee saveuser','url' => array('controller'=>'users','action' => 'edit',$user['User']['id'],$subscriberID),'escape'=>false,'update' => '#tr-'.$user['User']['id'], 'title' => 'Save','id' => 'submit-'.$user['User']['id']));?>
									<button class="btn btn-xs close-action closeuser" title="Cancel" type="reset">
										<i class="icon-remove bigger-120"></i>
									</button> 
								</a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
         <div class="model-body-inner-content">  
                  ...
                  <div class="form-group login-form-group relative">
                    <label class="col-sm-4 control-label">Role </label>    
                    <div class="col-sm-8 margin-bottom-zero form-filter-field  nopadding nopaddingleft nopaddingright no-border labelerror choosen_width">
                    	<?php echo $this->Form->input('role_id',array('class'=>'form-control invdrop selectitem','data-placeholder'=>"Select Role",'options'=>array(''=>'',$usergroups)));?>
                    </div>
                  </div>
                  <div class="form-group login-form-group "> 
...
$('.contentrow.row .ace').change(function(){
 ...
 $(document).ready(function(){
  ...
  /* ---   On edit  ----   */
    $('body').on('click', '.mobileUserView .dropdown-menu li .edit-row', function(){
    	
		$(this).parents('.mobileUserView').find('.on-load').hide();
		$(this).parents('.mobileUserView').find('.on-edit').fadeIn();
		$(this).parents('.mobileUserView').find('.mainbtn').hide();
				
	});
	$('body').on('click', '.mobileUserView .saveuser', function(){
		$(this).parents('.mobileUserView').find('.on-edit').hide();
		$(this).parents('.mobileUserView').find('.on-load').fadeIn();
		$(this).parents('.mobileUserView').find('.mainbtn').fadeIn();
		
	});
	
	$('body').on('click','.mobileUserView .closeuser', function(){
		$(this).parents('.mobileUserView').find('.on-edit').hide();
		$(this).parents('.mobileUserView').find('.on-load').fadeIn();
		$(this).parents('.mobileUserView').find('.mainbtn').fadeIn();
	});
