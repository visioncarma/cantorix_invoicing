...
<div class="page-content">
	<div class="page-header">
		...
		<div class="headernew col-lg-4 col-md-5 col-sm-4 col-xs-4 width-after-600"> <?php echo __('Manage Inventory');?> </div>
		<div class="col-lg-6 col-sm-12 col-md-12  col-xs-12 paddingleftrightzero pull-right">
			<?php if($permission['_create'] == '1'):?>
				<?php echo $this->Html->link('<i class="icon-plus"></i> Add Items',array('controller'=>'inventories','action'=>'add'),array('class'=>'btn btn-sm btn-success pull-right addbutton marginleft2 inventoryAdd col-xs-12 col-sm-12 col-md-12 col-lg-3','escape'=>FALSE));?>
		    <?php endif;?>
			...
			<?php echo $this->Html->link('Export Items <i class="icon-caret-down icon-on-right"></i>',array('controller'=>'inventories','action'=>'exportItems'),array('class'=>'btn btn-sm btn-success pull-right manageinventoryexport marginleft2 inventryExport col-xs-12 col-sm-12 col-md-12 col-lg-3','escape'=>FALSE));?>		
			<?php if($permission['_create'] == '1'):?>
				<?php echo $this->Html->link('Import Items <i class="icon-caret-down icon-on-right"></i>',array('controller'=>'inventories','action'=>'show_excel'),array('class'=>'btn btn-sm btn-success pull-right importbutton inventryimport col-xs-12 col-sm-12 col-md-12 col-lg-3','escape'=>FALSE));?>
			<?php endif;?>
		</div>
	</div>
	<!-- /.page-header -->
		...
		<div class="col-xs-12">
			<div class="table-responsive table-responsivenoscroll overflowvisible tablehidemobilee">
				...
				<div class="row margin-twenty-zero filterdivmob">
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 col-lg-2 col-md-12 col-sm-12 nopadding nopaddingmobile nopaddingleft nopaddingright">
					...
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 col-lg-2 col-md-12 col-sm-12 nopaddingmobile nopaddingleft nopaddingright">
					...
					<div class="form-group filed-left margin-bottom-zero form-filter-field col-xs-12 nopadding col-lg-2 col-md-12 col-sm-12 choosen_width">
						<?php echo $this->Form->input('quantity',array('div'=>false,'label'=>false,'data-placeholder'=>"Stock Status",'options'=>array(''=>'Stock Status','in-stock'=>'In Stock','out-stock'=>'Out Of Stock'),'class'=>'form-control invdrop'))?>
					</div>
					<div class="form-group filed-left margin-bottom-zero mobile_100">
						<?php echo $this->Js->submit('Filter', array('div'=>false,'class'=>'btn btn-sm btn-primary filter-btn taxgrpfilter form-control mobile_100','url' => array('controller'=>'inventories','action' => 'index'),'escape'=>false,'update' => '#pageContent'));?>
					</div>
					<div class="form-group filed-left margin-bottom-zero mobile_100">						
						<?php echo $this->Js->link('Reset',array('controller'=>'inventories','action'=>'index'),array('class'=>'btn btn-sm btn-primary filter-btn mobile_100','update'=>'#pageContent'));?>
					</div>
				</div>
				...
				<?php echo $this->Form->create('InvInventory');?>
                 ...
                 <span class="deleteicon delete" title="Delete Selected">
                 		<!--i class="icon-trash bigger-120" style="color:#B74635;"></i-->
                 		<?php 
							echo $this->Form->hidden('itemName',array('value'=>$itemName));
							echo $this->Form->hidden('minPrice',array('value'=>$minPrice));
							echo $this->Form->hidden('maxPrice',array('value'=>$maxPrice));
							echo $this->Form->hidden('inHand',array('value'=>$inHand));
							echo $this->Form->hidden('page',array('value'=>$pages));
						?>
						<?php if($permission['_delete'] == '1'):?>
						<?php echo $this->Js->submit('delete_selected.png', array('div'=>false,'url' => array('controller'=>'Inventories','action' => 'deleteAll'),'type'=>'submit','escape'=>false,'update' => '#pageContent','confirm'=>'Are you sure you want to remove the selected inventories?'));?>
                 		<?php endif;?>
                 </span>
                </div> 
				<table id="sample-table-1" class="table table-striped table-bordered table-hover editable-table">
					<thead>
						<tr>
							...
							<?php endif;?>
							<th>
								<?php echo $this->Paginator->sort('InvInventory.name','Item Name',array('update'=>'#pageContent','url'=>$url));?>
							</th>
							<th class="discr_width"><?php echo __('Description');?></th>
							<th ><?php echo __('Unit Type');?></th>
							<th class="textright paddingrightinven">
								<?php echo $this->Paginator->sort('InvInventory.list_price','Price',array('update'=>'#pageContent','url'=>$url));?>
							</th>
							<th class="textright paddingrightinven">
								<?php echo $this->Paginator->sort('InvInventory.current_stock','Onhand',array('update'=>'#pageContent','url'=>$url));?>
							</th>
					...
					<?php foreach($invInventories as $invInventory):?>
						<tr id = "tr-<?php echo $invInventory['InvInventory']['id'];?>">
							...
							<td><span class=""> <label>
									...
									<?php echo $this->Form->checkbox('deleteAll.'.$invInventory['InvInventory']['id'],array('div'=>false,'label'=>false,'class'=>'ace delete-m-row'));?>
							...
							<td ><span class="on-load"><?php if($invInventory['InvInventoryUnitType']['type_name']){ echo $invInventory['InvInventoryUnitType']['type_name'];}else{ echo "--";}?></span></td>
							<td class="textright paddingrightinven">
								<span class="on-load">
													    ...
													    <div class="modal-body">
													          <div class="model-body-inner-content">  
														           <div class="form-group login-form-group">
																         ...
																         <div class="col-sm-8">
																		    <p class="bold wordwrap marginzero"><?php echo $invInventory['InvInventory']['name'];?></p>
																		 </div>
																   </div>
																   <div class="form-group login-form-group"> 
																		<label class="col-sm-4 no-padding-top"> <?php echo __('Description');?> </label>   
																		<div class="col-sm-8">
																		  <p class="bold wordwrap marginzero"><?php echo $invInventory['InvInventory']['description']; ?></p>
																		</div>
																  </div> 
																  <div class="form-group login-form-group"> 
																		<label class="col-sm-4 no-padding-top"> <?php echo __('Price');?></label>   
																		<div class="col-sm-8">
																			 <p class="bold marginzero">
																			  	<?php $options = array('zero'=>'Free','places'=>'2','thousands'=>',','decimals'=>'.','wholeSymbol'=> '','wholePosition'=> 'before')?>
																				<?php echo $this->Number->currency($invInventory['InvInventory']['list_price'],$invInventory['CpnCurrency']['code'],$options);?>
																			 </p>
																		</div>
																  ...
																  <?php endif;?>
																	<?php foreach($customFields as $fieldId=>$fieldValue):?>
																		<div class="col-sm-12 form-group">
																			<label class="col-sm-4 control-label no-padding-top"> <?php echo $fieldValue;?> </label>
																			<div class="col-sm-8">
                                    ...
                                    <div class="pull-right">
										<!--<div class="inline position-relative">
											...
											<ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
														...
														<li>
															<?php if($permission['_update'] == 1):?>
																<?php echo $this->Html->link('<i class="icon-edit bigger-120"></i>',array('controller'=>'inventories','action'=>'editInventory',$invInventory['InvInventory']['id'],$pages,$itemName,$minPrice,$maxPrice,$inHand),array('class'=>'btn btn-xs btn-info edit edit-row on-load on-edit padding3','title'=>'Edit','escape'=>FALSE));?>
															<?php endif;?>
														</li>
	
														<li>
															<?php if($permission['_delete'] == 1):?>
																<?php echo $this->Html->link('<i class="icon-trash bigger-120"></i>',array('controller'=>'inventories','action'=>'delete',$invInventory['InvInventory']['id'],$pages,$itemName,$minPrice,$maxPrice,$inHand),array('class'=>'btn btn-xs btn-danger delete on-load padding3','title'=>'Delete','escape'=>FALSE), __('Are you sure you want to remove '.$invInventory['InvInventory']['name'].' from the system?'));?>
															<?php endif;?>
														</li>
													</ul>	
											<?php endif;?>
