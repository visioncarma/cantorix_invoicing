...
<html lang="en">
	<head>
		...
		<?php
	   		...
	   		echo $this->Html->script(array('jquery-2.0.3.min.js','jquery-ui-1.10.3.custom.min.js','bootstrap.min.js','chosen.jquery.min.js','typeahead-bs2.min.js','bootstrap-select.js','ace-elements.min.js','ace.min.js','jquery.validate.min.js','ace-extra.min.js','admin_dashboard_js.js','bootstrap-datepicker.min.js','jquery.placeholder.js','jquery.tablesorter.min.js'));
		...
		<div class="navbar navbar-default headerbg nav_header_border" id="navbar">	
			<div class="navbar-container" id="navbar-container">
				<div class="navbar-header pull-left">
					<?php 
						$menus = $this->Session->read('Auth.User.Menus');
						$homeLink = $this->Breadcrumb->getLink('Home');
						$details = $this->requestAction('subscribers/getSubscriberDetails');
						if(!empty($details['logo']['SbsSubscriberSetting']['invoice_logo'])) {
							...
							echo $this->Html->link($this->Html->image($imgLink.$details['logo']['SbsSubscriberSetting']['invoice_logo']),"$homeLink",array('escape'=>FALSE,'class'=>'navbar-brand'));
						} else {
							echo $this->Html->link($this->Html->image('logo_cantorix.png',array('alt'=>'logo')),"$homeLink",array('escape'=>FALSE,'class'=>'navbar-brand'));
						}
						echo '<div>'.$details['logo']['SbsSubscriberSetting']['text_logo'].'</div>';
					?>
					<!-- /.brand -->
				</div><!-- /.navbar-header -->
					...
					<ul class="nav ace-nav">
						<li class="light-blue">
							<a data-toggle="dropdown" href="#" class="dropdown-toggle">
								<?php if($details['user_detail']['User']['profile_picture_path']){?>
									<?php
										if(isset($_SERVER['HTTPS'] )  && $_SERVER['HTTPS'] != 'off' ){
										  $protocol_final = 'https';
										}else{
										...
										$imgLink = "$protocol_final".'://'."$http_hostname/";
									?>
								<img class="nav-user-photo" src="<?php echo $imgLink.$webroot_name.$details['user_detail']['User']['profile_picture_path'];?>"  alt="profilepic"/>
								<?php }else{
									echo $this->Html->image('avatar2.png',array('class'=>'nav-user-photo'));
								} ?>
								<span class="user-info">
									Welcome, <?php echo $details['user_detail']['User']['firstname']; ?>
								</span>
