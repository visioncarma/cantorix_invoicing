		...
		<div class="col-lg-8 paddingleftrightzero">
			...
			<div class="row marginleftrightzero">
				<div class="form-group  marginleftrightzero relative">
					...
					<div class="col-xs-12 col-sm-3 col-lg-6 marginleftrightzero paddingleftrightzero labelerror countrybilling max-height max-width">
						...
						<?php echo $this -> Form -> input('acr_client_id', array('id' => 'clientId', 'div' => false, 'label' => false, 'options' => array('' => '', $customer),'data-live-search' => 'true',  'class'=>'form-control','data-placeholder' => "Customer List"));
			...
			<div class="row marginleftrightzero" id = "currencyDiv">
				<div class="form-group marginleftrightzero relative">
					...
					<div class="col-xs-12 col-sm-3 col-lg-3 marginleftrightzero paddingleftrightzero labelerror countrybilling max-height max-width choosen_width">
						<?php echo $this -> Form -> input('cpn_currency_id', array('id' => 'invoiceCurrencySelect', 'div' => false, 'label' => false, 'options' => array('' => 'Select Currency', $currencyList), 'default' => $defaultCurrency, 'data-live-search' => 'true', 'class'=>'form-control invdrop','data-placeholder' => "Currency List",'disabled'=>'disabled'));
		...
		<table id="quote-table" class="quote-table table table-striped table-bordered table-hover editable-table margin-bottom-zero">
			<thead>
				<tr class="borderblue">
					...
					<th class="width90plus"><?php echo __('Discount %'); ?></th>
				...
				<tr id ="inventoryUpdateSelect-1">
					...
					<div class="form-group marginleftrightzero margin-bottom-zero">
						<div class="col-sm-10 marginleftrightzero paddingleftrightzero countrybilling max-height max-width">
							...
							<?php echo $this -> Form -> input('AcrClientInvoice.inventory.1', array('id' => 'inventory-1', 'div' => false, 'data-live-search'=>'true','label' => false,'data-placeholder'=>"Select inventory", 'class'=>'invdrop form-control','options' => array('' => '','-1'=>'Non Inventory Item',$inventoryList)));
					...
					<td>
					<div class="form-group marginleftrightzero margin-bottom-zero">
						<?php echo $this -> Form -> input('AcrClientInvoice.description.1', array('div' => false, 'label' => false, 'class' => 'col-sm-12 tabletextarea', 'type' => 'textarea','rows'=>'2', 'placeholder' => 'Inventory description', 'readonly' => 'readonly')); ?>
					</div></td>
					<td>
					<div class="form-group marginleftrightzero margin-bottom-zero">
					...
					<td>
					<div class="form-group marginleftrightzero margin-bottom-zero">
						<?php echo $this -> Form -> input('AcrClientInvoice.unit_rate.1', array('div' => false, 'label' => false, 'class' => 'col-sm-10 form-control textright', 'type' => 'text','autocomplete'=>'off', 'readonly' => 'readonly')); ?>
					</div></td>
					<td>
					<div class="form-group marginleftrightzero margin-bottom-zero">
						<?php echo $this -> Form -> input('AcrClientInvoice.discount_percent.1', array('div' => false, 'label' => false, 'class' => 'col-sm-10 form-control discountMax black textright', 'autocomplete'=>'off','type' => 'text','readonly' => 'readonly')); ?>
					</div></td>
					<td>
					<div class="form-group marginleftrightzero margin-bottom-zero">
		...
		<div class="modal-content">
			...
			<?php /*echo $this->Form->create('addInventory',array('id'=>'addInventory','role'=>'form','class'=>'form-horizontal popup'));*/?>
				<div class="modal-body">
					...
					<div class="col-sm-8">
						<?php echo $this->Form->input('addInventory.name-',array('div'=>false,'label'=>false,'class'=>'col-xs-10 env-name3 col-sm-5 form-control','autocomplete'=>'off','type'=>'text','id'=>'form-field-1','Placeholder'=>'Inventory name'));?>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<?php echo $this->Form->input('addInventory.description-',array('div'=>false,'label'=>false,'rows'=>'2','type'=>'textarea','class'=>'form-control col-xs-10 col-sm-5 itemdescription','id'=>'form-field-2','autocomplete'=>'off','Placeholder'=>'Description of the inventory'));?>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						...
						<span>
							<?php echo $this->Form->input('addInventory.list_price-',array('div'=>false,'label'=>false,'type'=>'text','id'=>'form-field-3','class'=>'col-xs-10 width70 env-price3 col-sm-5 form-control','autocomplete'=>'off','style'=>'width:37%','Placeholder'=>'Inventory price'));?>
						</span>
				...
				<div class="form-group currentstock" style="display: none;">
					...
					<div class="col-sm-8">
						<?php echo $this->Form->input('addInventory.current_stock-',array('div'=>false,'label'=>false,'type'=>'text','class'=>'form-control env-qty1 col-xs-10 col-sm-5','id'=>'form-field-4','autocomplete'=>'off','Placeholder'=>'Quantity of inventory  in stock'));?>
				...
				<div class="modal-footer paddingright8">
					<?php echo $this->Js->submit('Submit', array('id'=>'jsSubmit-','div'=>false,'class'=>'btn btn-info close-submit4','url' => array('controller'=>'inventories','action'=>'addInventory'),'escape'=>false,'update' => '#td-inventoryUpdateSelect-1'));?>
					...
					$('#addnewunittype-<?php echo $rowId;?>').on('show.bs.modal', function (e) {
						...
						$('.close-submit4').click(function(evt){
					    	...
					    	// if(value7.length === 0) {
						     ...
						     var value9=$.trim($(".env-price3").val());
					    	 if(value9.length === 0) {
					    	 	$('.popup-error3').show();
					    	 	evt.preventDefault();
						        $('#field').value();
...
<!--end of pop-->
	...
	<div class="row marginleftrightzero paddingbottom20 paddingtop25">
		<div class="row marginleftrightzero">
			<div class="form-group marginleftrightzero">
				<label  class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label marginleftrightzero paddingleftrightzero ">Terms and Conditions</label>
				<div class="col-lg-4 col-md-6 col-sm-10 col-xs-12 marginleftrightzero paddingleftrightzero">
					<?php echo $this->Form->input('AcrClientInvoice.terms',array('div'=>false,'label'=>false,'class'=>'termsandconditions width100','type'=>'textarea','value'=>$termsAndConditions))
					?>
		...
		<div class="row marginleftrightzero">
			<div class="form-group marginleftrightzero">
				<label  class="col-lg-2 col-md-2 col-sm-2 col-xs-12 control-label marginleftrightzero paddingleftrightzero">Customer Note</label>
				<div class="col-lg-4 col-md-6 col-sm-10 col-xs-12 marginleftrightzero paddingleftrightzero">
					<?php echo $this->Form->input('AcrClientInvoice.note',array('div'=>false,'label'=>false,'class'=>'form-control blacktext','type'=>'textarea','value'=>$defaultNotes))
					?>
				</div>
			</div>
		</div>
						 ...
						 <?php /*echo $this->Form->create('MailTemplate',array('class'=>'form-horizontal popup','role'=>'form','id'=>'MailTemplate'));*/?>
							...
							<div class="modal-footer">
								 <!--button class="btn btn-success addbutton left marginleftzero marginright14" type="button"> <i class="icon-zoom-in bigger-110"></i> Preview </button-->
								  <button class="btn btn-success addbutton left marginleftzero  marginright4 padding0 sendnow" style="font-size:13px;" title="Send" data-toggle="modal" data-target="#preview">
									<i class="icon-zoom-in bigger-110"></i> Preview
				 				  </button> 
								  <?php echo $this -> Form -> button(__('<i class="icon-share-alt bigger-110"></i> Send'), array('controller' => 'acr_client_invoices', 'action' => 'add', 'div' => false, 'class' => 'btn btn-info left marginleftzero marginright4 padding0 close-submit','style'=>'font-size:13px;', 'name' => 'Send Now', 'value' => '1')); ?>
