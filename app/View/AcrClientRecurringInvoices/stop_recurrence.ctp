<td class="select-each width-30-new">
		<label>
			...
			<span class="lbl"></span> 
							...
							<td class="title_role ewidth120 width-120-new expenseNO"><span class="on-load statusopn"><?php echo $acrClientInvoice['AcrClientInvoice']['invoice_number'];?></span>
							</td>
							<td class="title width-120-new"><span class="on-load"><?php echo date("$dateFormat",strtotime($acrClientInvoice['AcrClientRecurringInvoice']['invoice_start_date']));?></span>
							</td>
							<td class="title width-80-new"><span class="on-load"><?php echo date("$dateFormat",strtotime($acrClientInvoice['AcrClientRecurringInvoice']['invoice_end_date']));?></span>
							</td>
							<td class="title width-100-new textright"><span class="on-load"><?php $options = array('zero'=>'Free','places'=>'2','thousands'=>',','decimals'=>'.','wholeSymbol'=> '','wholePosition'=> 'before');?> 
							...
							<td class="title width-80-new" id = "td-<?php echo $acrClientInvoice['AcrClientRecurringInvoice']['id']?>"><span class="on-load "><?php echo $acrClientInvoice['AcrClientRecurringInvoice']['status'];?></span>
							</td>
							<td class="title width-120-new" style="padding-right: 7%;">
...
?>
								<?php }else{
echo $this->Js->link('<i class="icon-play bigger-120"></i>', array (
	'controller' => 'acr_client_recurring_invoices',
	'action' => 'startRecurrence',
	$acrClientInvoice['AcrClientRecurringInvoice']['id']
), array (
	'class' => 'btn btn-xs btn-danger edit edit-row on-load pull-right',
	'escape' => FALSE,
	'update' => '#tr-' . $acrClientInvoice['AcrClientRecurringInvoice']['id'],
	'title' => 'Start Recurrence',
	'confirm' => 'Do you want to start recurrence for Invoice# ' . $acrClientInvoice['AcrClientInvoice']['invoice_number'] . '?'
));
								} ?>
								<?php
