	...
	<div class="row">
		<div class="col-xs-12">
			<div class="table-responsive tablemobile_overflowhidd">
				...
				<table id="sample-table-1" class="table table-striped table-bordered table-hover table_fixed_new viewusergroup">
					<thead>
						<tr>
							<th>Module Name</th>
							<th>View</th>
				...
				<?php echo $this->Form->create('User',array('url'=>array('controller'=>'users','action'=>'edit',$id,$user['User']['id']),'inputDefaults'=>array('div'=>FALSE,'label'=>FALSE,'class'=>'on-edit')));?>
				<div class="table-responsive tablemobile_overflowhidd">		
				<table id="sample-table-1" class="table table-striped table-bordered table-hover editable-table  table_fixed_new rol_users">
					<thead>
						<tr>
							<th>User Name</th>
							<th>Email</th>
							<th >Role</th>
							<th >Active</th>
							<?php /*if($userPermission['_update'] == 1 || $userPermission['_delete'] == 1):*/?><th>Action</th><?php /*endif;*/?>
						</tr>
					</thead>
					...
					<tbody>
						...
						<tr id="tr-<?php echo $user['User']['id'];?>">
							<td>
								...
								<?php echo $this->Form->input($user['User']['id'].'.username',array('placeholder'=>'User Name','class'=>'width100 on-edit form-control','value'=>$user['User']['username'],'disabled'=>'disabled'));?>
							...
							<td>
								...
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<?php echo $this->Form->input($user['User']['id'].'.email',array('placeholder'=>'Email','class'=>'width100 on-edit form-conrtol','value'=>$user['User']['email'],'disabled'=>'disabled'));?>
							...
							<td>
								...
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<span class="on-edit inlineeditselectpick usergroupselect usergroupviewselect">	
									<?php echo $this->Form->input($user['User']['id'].'.aro_id',array('class'=>'selectpicker','data-placeholder'=>'Role','options'=>array($user_groups),'default'=>$groupName['Aro']['id']));?>
								</span>
								<?php endif;?>
							</td>
							<td  ><span class="on-load"><?php if($user['User']['active'] == 'Y') {echo 'Yes';} else { echo 'No'; }?></span>
								<?php if($user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
								<span class="on-edit inlineeditselectpick userselect usergroupviewselect">
									<?php echo $this->Form->input($user['User']['id'].'.active',array('class'=>'selectpicker','data-placeholder'=>'Role','options'=>array('Y'=>'Yes','N'=>'No'),'default'=>$user['User']['active']));?>
								</span>
								<?php endif;?>
							</td>
							<td>
							<div class="btn-group">
								...
								<a href="javascript:void(0)" class="save-row on-edit" style="display:none;">
									<?php echo $this->Js->submit('', array('div'=>false,'class'=>'save-row submit greennn','url' => array('controller'=>'users','action' => 'edit',$user['User']['id'],$subscriberID,'view'),'escape'=>false,'update' => '#tr-'.$user['User']['id'], 'title' => 'Save','id' => 'submit-'.$user['User']['id']));?>
							...
							<!--m0bile view-->
								...
								<?php endif;?>
											...
											<li><?php if($permissions['_update'] == 1 && $user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
									<a class="btn btn-xs btn-info edit edit-row on-load" title="Edit">
										<i class="icon-edit bigger-120"></i>
									</a>
								<?php endif;?> </li>
										<li> <?php if($permissions['_delete'] == 1 && $user['User']['id'] != $adminUser['Aro']['foreign_key']):?>
									<?php echo $this->Js->link('<i class="icon-trash bigger-120"></i>',
												array('controller'=>'usergroups','action' => 'deleteUser',$id,$user['User']['id'],$subscriberID,$page), array('update'=>'#pageContent','class'=>'btn btn-xs btn-danger delete on-load','title'=>'Delete','escape'=>FALSE,'confirm'=>'Are you sure want to delete user?')); ?>		
								<?php endif;?>
								 </li>
			...
			<div class="row clear col-xs-12 paginationmaindiv">
					...
					<div class="col-sm-6">
						<div class="dataTables_paginate paging_bootstrap">
							<ul class="pagination">
								<?php
									$url = array('action'=>'view',$id,$subscriberID);
