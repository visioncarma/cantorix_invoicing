...
<?php $page = $this->Paginator->current('Aro');?>
		...
		<div class="page-header">
			...
			<div class="col-lg-8 col-md-7 col-sm-8 col-xs-8 no-padding-left no-padding-right width-after-600">
				<div class="widthauto paddingleftrightzero pull-right buttonrightwidth padding-right-3-480 ">
					<?php echo $this->Html->link('<i class="icon-plus"></i>Add New Role',array('controller'=>'usergroups','action'=>'add'),array('class'=>'btn btn-sm btn-success pull-right addbutton width-after-400','escape'=>FALSE));?>
				</div>
			</div>	
			<?php endif;?>
		</div>
					...
					<div  class="row magin-delete-all">
		                 <?php 
		                 	if($permission['_delete'] == '1') {
								echo $this->Form->submit('delete_selected.png',array('url'=>array('action'=>'manageUsergroup'),'class'=>'deleteicon delete','type'=>'submit','title'=>'Delete Selected','onclick'=>"return confirm('Are you sure you want to delete selected user roles and it\'s users ?')"));
							}
						 ?>
		            </div> 
								...
								<td>
									<div class="btn-group hidden-xs hidden-sm visible-md visible-lg">
										<?php echo $this->Html->link('<i class="icon-zoom-in bigger-120"></i>',
												array('action' => 'view', $usergroup['Aro']['id'],$subscriberID), array('escape'=>FALSE,'class'=>'btn btn-xs btn-success view on-load','title'=>'View')); ?>
										<?php if($permission['_update'] == '1'):?>
											<?php echo $this->Html->link('<i class="icon-edit bigger-120"></i>',
												array('action' => 'edit', $usergroup['Aro']['id'],$subscriberID,$page), array('escape'=>FALSE,'class'=>'btn btn-xs btn-info edit on-load','title'=>'Edit')); ?>
										<?php endif;?>
										<?php if($permission['_delete'] == '1' && $adminUsergroup['Aro']['id'] != $usergroup['Aro']['id']):?>
											...
											<?php echo $this->Form->postLink('<button class="btn btn-xs btn-danger delete on-load borderwidth" title="Delete"><i class="icon-trash bigger-120"></i></button>',
