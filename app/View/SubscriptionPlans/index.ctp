				...
				<table id="sample-table-1" class="table table-striped table-bordered table-hover rt rt1">
					<thead>
						<tr>
							...
							<th class="admin-text-right padding-right-25"><?php echo $this->Paginator->sort('cost','Cost'); ?></th>
					...
					<tbody>
						<?php foreach ($cpnSubscriptionPlans as $cpnSubscriptionPlan): ?>
							<tr>
								...
								<td class="admin-text-right padding-right-25">
									<?php /*echo  number_format((float)$cpnSubscriptionPlan['CpnSubscriptionPlan']['cost'], 2, '.', '');*/?>
									<?php echo $this->Number->currency($cpnSubscriptionPlan['CpnSubscriptionPlan']['cost'],$getSetting['CpnSetting']['currency_code']);?>
								</td>
								<td >
