 ...
 */
	...
	public function getSubscriberPayment($subscriberId = null,$month =null){
			if($subscriberId){
				$i = $month -1;
				$j = $i-1;
				$getPaymentForCurrentMonth = $this->find('first',array('conditions'=>array('CpnSubscriberInvoiceDetail.sbs_subscriber_id'=>$subscriberId,'CpnSubscriberInvoiceDetail.last_payment_date like'=>date("Y-m").'%')));
				$getPaymentForLastMonth = $this->find('first',array('conditions'=>array('CpnSubscriberInvoiceDetail.sbs_subscriber_id'=>$subscriberId,'CpnSubscriberInvoiceDetail.last_payment_date like'=>date("Y-m",strtotime( date( 'Y-m-01' )." -$i months")).'%')));
				$getPaymentForLastToLastMonth = $this->find('first',array('conditions'=>array('CpnSubscriberInvoiceDetail.sbs_subscriber_id'=>$subscriberId,'CpnSubscriberInvoiceDetail.last_payment_date like'=>date("Y-m",strtotime( date( 'Y-m-01' )." -$j months")).'%')));
				if((!$getPaymentForCurrentMonth) && (!$getPaymentForLastMonth) && (!$getPaymentForLastToLastMonth)){
					return true;
				}else{
					return false;
				}
