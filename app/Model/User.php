 ...
 */
	...
	public function addSubscriber ($sbs_subscriber_id, $password){
		...
		if($lastuser) {
			...
			$aro = $Acl->Aro;
			$parentIDDD = $aro->find('first',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.foreign_key'=>NULL,'Aro.sbs_subscriber_id'=>0,'Aro.alias'=>'Subscriber')));
			$data     = null;
			$aro->create();
			$data['alias']       = 'Admin';
			$data['parent_id']   = $parentIDDD['Aro']['id'];
 ...
 */	
	public function addUserGroup($data = NULL,$user_type = NULL, $aro_id = NULL ,$subscriber_id = NULL) {
		...
		if($user_type == 'Super Admin') {
			$parentIDDD = $aro->find('first',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.foreign_key'=>NULL,'Aro.sbs_subscriber_id'=>0,'Aro.alias'=>'Admin')));
			$save['Aro']['parent_id'] = $parentIDDD['Aro']['id'];
		} else {
			$parentIDDD = $aro->find('first',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.foreign_key'=>NULL,'Aro.sbs_subscriber_id'=>0,'Aro.alias'=>'Subscriber')));
			$save['Aro']['parent_id'] = $parentIDDD['Aro']['id'];
		}
 ...
 */	
	public function getUsergroups($user_type = NULL,$subscriber_id = NULL) {
		...
		if($user_type == 'Subscriber') {
			$parenttIDDDD = $Aro->find('first',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.foreign_key'=>NULL,'Aro.sbs_subscriber_id'=>0,'Aro.alias'=>'Subscriber'),'fields'=>array('Aro.id')));
			$usergroups = $Aro->find('list',array('conditions'=>array('Aro.parent_id'=>$parenttIDDDD['Aro']['id'],'Aro.foreign_key'=> NULL,'Aro.sbs_subscriber_id'=>$subscriber_id),'fields'=>array('Aro.id','Aro.alias')));
		} else {
			...
			$usergroups = $Aro->find('list',array('conditions'=>array('Aro.parent_id'=>$parenttIDDDD['Aro']['id'],'Aro.foreign_key'=> NULL),'fields'=>array('Aro.id','Aro.alias')));
		}
		return $usergroups;
	}
 ...
 */
	public function archiveUser($subscriberId = NULL) {
		...
		$aco = $Acl->Aco;
		$aro->recursive = -1;
        $parentAros = $aro->find('all',array('conditions'=>array('Aro.parent_id'=>NULL,'Aro.model'=>NULL,'Aro.foreign_key'=>NULL)));
        debug($parentAros);
		...
		$CpnPaymentMethod = ClassRegistry::init('CpnPaymentMethod');
        ...
        $paymentMethods = $CpnPaymentMethod->find('all');
        ...
        $CpnPaymentMethod->useDbConfig  = $archiveConfigVar;
        $CpnPaymentMethod->schemaName = $archiveDatabase;
        ...
        foreach ($paymentMethods as $paymentMethod) {
            $archivePaymentMethods = $CpnPaymentMethod->find('first',array('conditions'=>array('CpnPaymentMethod.id'=>$paymentMethod['CpnPaymentMethod']['id']),'fields'=>array('CpnPaymentMethod.id')));
            if(empty($archivePaymentMethods)) {
                $CpnPaymentMethod->save($paymentMethod);
            }
        }
		...
		foreach ($parentAros as $parentAro) {
		    $parentAros = $aro->find('first',array('conditions'=>array('Aro.id'=>$parentAro['Aro']['id'])));
		    if(empty($parentAros)) {
		          if($aro->save($parentAro)) {
                    debug('parent saved');
                } else {
                    debug('parent not saved');
                }    
		    }
		}
		...
		debug($aros);
        debug($aro->useDbConfig);
        debug($aro->schemaName);
		foreach($aros as $aroInsert) {
		    debug($aroInsert);
			if($aro->save($aroInsert)) {
			    debug('saved');
			} else {
			    debug('Not Saved');
			}
 ...
 */
	public function restoreArchivedUser($subscriberId = NULL) {
		...
		/*Aro Insert*/
		foreach($aros as $aroInsert) {
		    $aroExistt = $aro->findById($aroInsert['Aro']['id']);
            if(empty($aroExistt)) {
                $aro->save($aroInsert);    
            }
		}
 ...
 * **/
 	 ...
 	 * @Since 19 Sep 2014
 	 * @Version v.1
 	 * @Method Check whether fraud usergroup.
 	 * @Return True if fraud, false if not fraud usergroup
 	 * **/
 	public function checkFraudUsergroupAdmin($id = NULL) {
 		$Aro = ClassRegistry::init('Aro');
 		$userGroup = $Aro->find('first',array('conditions'=>array('Aro.id' => $id, 'Aro.sbs_subscriber_id' => 0,'Aro.foreign_key'=> NULL)));
 		if(empty($userGroup)) {
 			 return TRUE;
 		} else {
 			return FALSE;
