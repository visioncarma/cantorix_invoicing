 ...
 */
	...
	public function getListOfInventory($subscriberId = null){
		if($subscriberId){
			$inventory = $this->find('list',array(
			'conditions'=>array(
			'OR'=>array(
						array(
								'InvInventory.sbs_subscriber_id'=>$subscriberId,
								'InvInventory.current_stock >='=>'1',
								'InvInventory.track_quantity'=>'Y'
						),
						array(
							'InvInventory.sbs_subscriber_id'=>$subscriberId,
							'InvInventory.track_quantity'=>'N'
						)
			),
					
								
								),
			'fields'=>array('InvInventory.id','InvInventory.name'),
	...
	public function getAllInventorySubscriber($subscriberId = null) {
		return $this->find('list',array('conditions'=>array('InvInventory.sbs_subscriber_id'=>$subscriberId),'fields'=>array('InvInventory.id','InvInventory.name'),'order'=>array('InvInventory.name ASC')));
	}
	...
	public function addInventory($data = null,$subscriberId = null,$rowId = null){
	...
	public function addInventoryFromEditQuote($data = null,$subscriberId = null,$rowId = null){
		if($data && $subscriberId){
			$addInventory->data = null;
			$this->create();
			$addInventory->data['InvInventory']['name'] 						= $data['name'][$rowId];
			$addInventory->data['InvInventory']['description'] 					= $data['description'][$rowId];
			$addInventory->data['InvInventory']['list_price'] 					= $data['list_price'][$rowId];
			if($data['track'][$rowId]){
				$addInventory->data['InvInventory']['track_quantity'] 			= "Y";
				$addInventory->data['InvInventory']['current_stock'] 			= $data['current_stock'][$rowId];
			}else{
				$addInventory->data['InvInventory']['track_quantity'] 			= "N";
			}
			$addInventory->data['InvInventory']['sbs_subscriber_id'] 			= $subscriberId;
			$addInventory->data['InvInventory']['inv_inventory_unit_type_id'] 	= $data['unitType'][$rowId];
			$addInventory->data['InvInventory']['cpn_currency_id'] 				= $data['currency'][$rowId];
			if($data['tax_inventory'][$rowId]){
				$taxGroupId = explode('-',$data['tax_inventory'][$rowId]);
				if($taxGroupId['1']){
					$addInventory->data['InvInventory']['sbs_subscriber_tax_group_id'] 	= $taxGroupId['1'];
				}else{
					$addInventory->data['InvInventory']['sbs_subscriber_tax_id'] 		= $data['tax_inventory'][$rowId];
				}
			}
			if($this->save($addInventory->data)){
				$lastInventoryId  = $this->getLastInsertID();
   ...
   public function checkInventoryExistUnitType($unit_type_id=null,$subscriberId=null){
    	$inventoryRecord = $this->find('first',array('conditions'=>array('InvInventory.inv_inventory_unit_type_id'=>$unit_type_id,'InvInventory.sbs_subscriber_id'=>$subscriberId),'fields'=>array('InvInventory.id')));
        if($inventoryRecord){
        	return true;
        }else{
        	return false;
