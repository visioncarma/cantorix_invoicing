			...
			foreach($contactArray as $key=>$val){
				if(!$val['Contact Name *']){
					...
					if(($val['Primary Contact'] == 'Yes') || ($val['Primary Contact'] == 'yes') || ($val['Primary Contact'] == 'Y') ||($val['Primary Contact'] == 'y')){
						$getClientContactDetails = $this->find('first',array('conditions'=>array('AcrClientContact.acr_client_id'=>$newClientId,'AcrClientContact.is_primary'=>'Y')));
						if($getClientContactDetails){
					...
					if($this->save($saveArray->data)){
						...
						$arraySave['contactSuccess'] = $countOfContact;
					}
				}
			}
			if($arraySave){
