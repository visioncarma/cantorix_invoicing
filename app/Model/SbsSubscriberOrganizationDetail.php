 ...
 */
	...
	public function getHomeCurrency($currencyId = null,$organizationId = null){
		if($organizationId && $currencyId){
			$organizationDetails  =$this->find('first',array('conditions'=>array('SbsSubscriberOrganizationDetail.id'=>$organizationId,'SbsSubscriberOrganizationDetail.cpn_currency_id'=>$currencyId),'fields'=>array('SbsSubscriberOrganizationDetail.id','SbsSubscriberOrganizationDetail.cpn_currency_id')));
			if($organizationDetails){
				return $organizationDetails;
			}else{
				return false;
